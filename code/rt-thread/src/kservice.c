 ...
 * @return errno
 */
rt_err_t __fast rt_get_errno(void)
{
    rt_thread_t tid;
 ...
 */
void __fast rt_set_errno(rt_err_t error)
{
    rt_thread_t tid;
 ...
 */
void __fast *rt_memset(void *s, int c, rt_ubase_t count)
{
 ...
 */
void __fast *rt_memcpy(void *dst, const void *src, rt_ubase_t count)
{
#ifdef RT_USING_TINY_SIZE
 ...
 * @return the address of destination memory
 */
void __fast *rt_memmove(void *dest, const void *src, rt_ubase_t n)
{
 ...
 * @return the result
 */
rt_int32_t __fast rt_memcmp(const void *cs, const void *ct, rt_ubase_t count)
{
 ...
 */
char __fast *rt_strstr(const char *s1, const char *s2)
{
    int l1, l2;
 ...
 * @return the result
 */
rt_uint32_t __fast rt_strcasecmp(const char *a, const char *b)
{
    int ca, cb;
 ...
 * @return the result
 */
char __fast *rt_strncpy(char *dst, const char *src, rt_ubase_t n)
{
    if (n != 0)
    {
        char *d = dst;
 ...
 * @return the result
 */
rt_int32_t __fast rt_strncmp(const char *cs, const char *ct, rt_ubase_t count)
{
 ...
 * @return the result
 */
rt_int32_t __fast rt_strcmp(const char *cs, const char *ct)
{
    while (*cs && *cs == *ct)
        cs++, ct++;
 ...
 */
rt_size_t __fast rt_strnlen(const char *s, rt_ubase_t maxlen)
{
    const char *sc;
 ...
 * @return the length of string
 */
rt_size_t __fast rt_strlen(const char *s)
{
    const char *sc;
 ...
 */
char __fast *rt_strdup(const char *s)
{
...
static inline int32 divide(long long  *n, int32 base)
{
    long long  res;
...
#else
rt_inline rt_int32_t __fast divide(rt_int32_t *n, rt_int32_t base)
{
    rt_int32_t res;
...
#endif
rt_inline int __fast skip_atoi(const char **s)
{
    register int i = 0;
    while (isdigit(**s))
        i = i * 10 + *((*s)++) - '0';
...
#endif
{
    ...
    if(field_width > (precision + 1))
    {
        ...
        buf = print_number(buf, end, temp, 10, field_width, -1, type);  // (str, end, num, base, field_width, precision, flags);
#else
        buf = print_number(buf, end, temp, 10, -1, type);
#endif
    ...
    *buf = '.';
    ...
    if (f == 0)
    {
        ...
        buf = print_number(buf, end, 0, 10, temp_f_pre, -1, SIGN|ZEROPAD);
#else
        temp_f_pre = 6;
        buf = print_number(buf, end, 0, 10, temp_f_pre, SIGN);
#endif
    }
    else
    {
#ifdef RT_PRINTF_PRECISION
        if(precision > 0 ) {
            if (precision > PRINTF_FLOAT_DECIMAL_MAX){

                precision = PRINTF_FLOAT_DECIMAL_MAX;
            }
        }else{
            precision = PRINTF_FLOAT_DECIMAL_DEFAULT;
        }
        for(i = precision; i > 0; i--){
            temp_pre = 10 *temp_pre;
        }
#else
    for(i = PRINTF_FLOAT_DECIMAL_DEFAULT; i > 0; i--){
            temp_pre = 10 *temp_pre;
        }
#endif
        temp = (int)(f*temp_pre);
...
#ifdef RT_PRINTF_PRECISION
        buf = print_number(buf, end, temp, 10, precision, -1, SIGN|ZEROPAD);
#else
        buf = print_number(buf, end, temp, 10,PRINTF_FLOAT_DECIMAL_DEFAULT, SIGN|ZEROPAD);
#endif
    }
    return buf;
                        ...
                        va_list     args)
{
    ...
    for (; *fmt ; ++fmt)
    {
        ...
        case 'f':
            flags |= SIGN;
#ifdef RT_PRINTF_PRECISION
            str = print_float(str, end, field_width, precision, va_arg(args,double),flags);
#else
            str = print_float(str, end, field_width, va_arg(args,double),flags);
#endif
            continue;
#endif
        default:
 ...
 */
void __fast *rt_malloc_align(rt_size_t size, rt_size_t align)
{
    void *align_ptr;
    void *ptr;
 ...
 */
void __fast rt_free_align(void *ptr)
{
    void *real_ptr;
 ...
 */
void __fast rt_assert_set_hook(void (*hook)(const char *ex, const char *func, rt_size_t line))
{
    rt_assert_hook = hook;
}
 ...
 */
void __fast rt_assert_handler(const char *ex_string, const char *func, rt_size_t line)
{
...
/* Thread-safe memcpy function for high speed */
#include <tih/memcpy_hw.h>
...
/* Thread-safe memset function for high speed, c: not used */
void __fast *xmemset(void *s, int c, size_t n)  // only support c = 0
{
    if (n > 64)
    {
        rt_base_t level;
...
void __fast *xmemcpy(void *dest, const void *src, size_t n)
{
    rt_base_t level;
    if (n > 64)
    {
