 ...
 */
void __fast rt_timer_timeout_sethook(void (*hook)(struct rt_timer *timer))
{
...
static void __fast _rt_timer_init(rt_timer_t timer,
...
static rt_tick_t __fast rt_timer_list_next_timeout(rt_list_t timer_list[])
{
...
rt_inline void __fast _rt_timer_remove(rt_timer_t timer)
{
    int i;
 ...
 */
void __fast rt_timer_init(rt_timer_t  timer,
                   const char *name,
                   void (*timeout)(void *parameter),
                   void       *parameter,
                   rt_tick_t   time,
 ...
 */
rt_err_t __fast rt_timer_detach(rt_timer_t timer)
{
 ...
 */
rt_timer_t __fast rt_timer_create(const char *name,
                           void (*timeout)(void *parameter),
                           void       *parameter,
                           rt_tick_t   time,
                           rt_uint8_t  flag)
 ...
 * @return the operation status, RT_EOK on OK; RT_ERROR on error
 */
rt_err_t __fast rt_timer_delete(rt_timer_t timer)
{
    register rt_base_t level;
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 */
rt_err_t __fast rt_timer_start(rt_timer_t timer)
{
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 */
rt_err_t __fast rt_timer_stop(rt_timer_t timer)
{
    register rt_base_t level;
 ...
 * @return RT_EOK
 */
rt_err_t __fast rt_timer_control(rt_timer_t timer, int cmd, void *arg)
{
    /* timer check */
    RT_ASSERT(timer != RT_NULL);
 ...
 */
void __fast rt_timer_check(void)
{
    struct rt_timer *t;
 ...
 */
rt_tick_t __fast rt_timer_next_timeout_tick(void)
{
 ...
 * corresponding timeout function will be invoked.
 */
void __fast rt_soft_timer_check(void)
{
    rt_tick_t current_tick;
    rt_list_t *n;
    struct rt_timer *t;
...
static void __fast rt_thread_timer_entry(void *parameter)
{
 ...
 */
void __fast rt_system_timer_init(void)
{
    int i;
