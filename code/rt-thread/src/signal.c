...
static void __fast _signal_default_handler(int signo)
{
    dbg_log(DBG_INFO, "handled signo[%d] with default action.\n", signo);
    return ;
}

static void __fast _signal_entry(void *parameter)
{
 ...
 */
static void __fast _signal_deliver(rt_thread_t tid)
{
    rt_ubase_t level;
...
void __fast rt_signal_mask(int signo)
{
    rt_base_t level;
    rt_thread_t tid = rt_thread_self();
...
void __fast rt_signal_unmask(int signo)
{
    rt_base_t level;
    rt_thread_t tid = rt_thread_self();
...
int __fast rt_signal_wait(const rt_sigset_t *set, rt_siginfo_t *si, rt_int32_t timeout)
{
    int ret = RT_EOK;
    rt_base_t   level;
    rt_thread_t tid = rt_thread_self();
...
void __fast rt_thread_handle_sig(rt_bool_t clean_state)
{
    rt_base_t level;
...
void __fast rt_thread_alloc_sig(rt_thread_t tid)
{
    int index;
    rt_base_t level;
...
void __fast rt_thread_free_sig(rt_thread_t tid)
{
    rt_base_t level;
...
int __fast rt_thread_kill(rt_thread_t tid, int sig)
{
    siginfo_t si;
    rt_base_t level;
    struct siginfo_node *si_node;
