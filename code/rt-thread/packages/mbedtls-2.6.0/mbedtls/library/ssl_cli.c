...
static int ssl_write_client_hello( mbedtls_ssl_context *ssl )
{
    ...
    /* Set protocol type for GMSSL or TLS */
    if( ssl->conf->max_major_ver == MBEDTLS_GMSSL_MAJOR_VERSION_1 &&
        ssl->conf->max_minor_ver == MBEDTLS_GMSSL_MINOR_VERSION_1 )
    {
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        ...
        if( md_alg != MBEDTLS_MD_NONE )
        {
            ...
            if( ( ret = mbedtls_md_setup( &ctx,
                                     mbedtls_md_info_from_type( md_alg ), MBEDTLS_MD_HW_MD ) ) != 0 )
            {
