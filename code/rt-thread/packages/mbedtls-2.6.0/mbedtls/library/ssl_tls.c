                     ...
                     unsigned char *dstbuf, size_t dlen )
{
    ...
    if( ( ret = mbedtls_md_setup( &md_ctx, md_info, MBEDTLS_MD_HW_HMAC ) ) != 0 )
        return( ret );
...
int mbedtls_ssl_derive_keys( mbedtls_ssl_context *ssl )
{
...
#if defined(MBEDTLS_SM3_C)
        if( ssl->minor_ver == MBEDTLS_GMSSL_MINOR_VERSION_1 && ssl->major_ver == MBEDTLS_GMSSL_MAJOR_VERSION_1 )
        {
    ...
    else
    {
        ...
        if( ( ret = mbedtls_md_setup( &transform->md_ctx_enc, md_info, MBEDTLS_MD_HW_HMAC ) ) != 0 ||
            ( ret = mbedtls_md_setup( &transform->md_ctx_dec, md_info, MBEDTLS_MD_HW_HMAC ) ) != 0 )
        {
...
static int ssl_encrypt_buf( mbedtls_ssl_context *ssl )
{
    ...
    if( mode == MBEDTLS_MODE_CBC )
    {
         ...
         */
        if( ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_2 || ssl->protocol == 0 )
        {
            /*
             * Generate IV
             */
...
//            size_t j,
             ...
             * Note:
             *
             *  Through test, the listed code has an influence up on
             *  muiti-thread TLS task running.
             *
             *  Because of hardware offload, cache attacks don't exist, so we
             *  comment the listed code to remark it.
             *                             by TIH on 2019-11-21
             */
...
static void ssl_swap_epochs( mbedtls_ssl_context *ssl )
{
    ...
    if( ssl->transform_out != NULL &&
        ( ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_2 || ssl->protocol == 0 ) )
    {
                       ...
                       mbedtls_ssl_config *conf )
{
     ...
     */
    if( ( ssl->in_buf = mbedtls_calloc( 1, len + PADDING_OF_PRIVATE ) ) == NULL ||
        ( ssl->out_buf = mbedtls_calloc( 1, len + PADDING_OF_PRIVATE ) ) == NULL )
    {
        MBEDTLS_SSL_DEBUG_MSG( 1, ( "alloc(%d bytes) failed", len + PADDING_OF_PRIVATE ) );
        mbedtls_free( ssl->in_buf );
        ssl->in_buf = NULL;
        return( MBEDTLS_ERR_SSL_ALLOC_FAILED );
    }

    ssl->in_buf += PADDING_OF_PRIVATE;
    ssl->out_buf += PADDING_OF_PRIVATE;
...
#if defined(MBEDTLS_GMSSL)
        case MBEDTLS_SSL_PRESET_SUITE_GM:

            conf->min_major_ver = MBEDTLS_GMSSL_MAJOR_VERSION_1;
            conf->min_minor_ver = MBEDTLS_GMSSL_MINOR_VERSION_1; /* GMTLS */
            conf->max_major_ver = MBEDTLS_GMSSL_MAJOR_VERSION_1;
            conf->max_minor_ver = MBEDTLS_GMSSL_MINOR_VERSION_1;
...
#if defined(MBEDTLS_GMSSL)
        case MBEDTLS_SSL_PRESET_SUITE_GE_AND_GM:
