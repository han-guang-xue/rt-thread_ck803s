...
read_record_header:
...
#if defined(MBEDTLS_GMSSL)
    if( major == MBEDTLS_GMSSL_MAJOR_VERSION_1 && minor == MBEDTLS_GMSSL_MINOR_VERSION_1 )
    {
        ssl->protocol = 0; // GMSSL
    }
    else
    {
        ssl->protocol = 1; // Generic TLS
    }
#endif
    ...
    ciphersuites = ssl->conf->ciphersuite_list[ssl->minor_ver];
    ciphersuite_info = NULL;
#if defined(MBEDTLS_DEBUG_CIPHERSUITES)
{
	static int ciphersuites_last = 0;
	int ciphersuites_current = 0;
	int select_flag = 0;
	...
	for( i = 0; ciphersuites[i] != 0; i++ ){
...
#endif
        {
        	if( p[0] != ( ( ciphersuites[i] >> 8 ) & 0xFF ) ||
            	...
            	ciphersuites_current++;
            }
        	if( ciphersuites_current < ciphersuites_last ){
            	continue;
            }else{
...
#if defined(MBEDTLS_GMSSL)
//#if defined(MBEDTLS_KEY_EXCHANGE__SOME__ECC_SM2_ENABLED)
...
//#endif/*MBEDTLS_KEY_EXCHANGE__SOME__ECC_SM2_ENABLED*/
#endif /* MBEDTLS_GMSSL */
    ...
    defined(MBEDTLS_GMSSL)
        ...
        if( md_alg != MBEDTLS_MD_NONE )
        {
             ...
             * };
             */
            if( ( ret = mbedtls_md_setup( &ctx, md_info, MBEDTLS_MD_HW_MD ) ) != 0 )
            {
...
#if defined(MBEDTLS_GMSSL)
#if defined(MBEDTLS_KEY_EXCHANGE_ECC_SM2_ENABLED)
...
#endif /*MBEDTLS_KEY_EXCHANGE_ECC_SM2_ENABLED*/
#endif
...
#if defined(MBEDTLS_SSL_PROTO_SSL3) || defined(MBEDTLS_SSL_PROTO_TLS1) || \
    defined(MBEDTLS_SSL_PROTO_TLS1_1)
    if( ssl->minor_ver != MBEDTLS_SSL_MINOR_VERSION_3 && ssl->protocol != 0x0 )
    {
        md_alg = MBEDTLS_MD_NONE;
        hashlen = 36;
