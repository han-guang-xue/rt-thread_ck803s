...
/**
 * \brief          SM4-CFB128 buffer encryption/decryption.
 *
...
/**
 * \brief          SM4-CFB8 buffer encryption/decryption.
 *
 * Note: Due to the nature of CFB you should use the same key schedule for
 * both encryption and decryption. So a context initialized with
 * mbedtls_sm4_setkey_enc() for both MBEDTLS_SM4_ENCRYPT and MBEDTLS_SM4_DECRYPT.
 ...
 * \param ctx      SM4 context
 * \param mode     MBEDTLS_SM4_ENCRYPT or MBEDTLS_SM4_DECRYPT
 * \param length   length of the input data(n * 16bytes)
 ...
 * \brief          SM4-CFB buffer encryption/decryption.
 *
 * Note: Due to the nature of XTS you should use the same key schedule for
 * both encryption and decryption. So a context initialized with
 * mbedtls_sm4_setkey_enc() for both MBEDTLS_SM4_ENCRYPT and MBEDTLS_SM4_DECRYPT.
 *
 * \note           Upon exit, the content of the IV is updated so that you can
 ...
 * \return         0 if successful
 */
int mbedtls_sm4_crypt_xts( mbedtls_sm4_context *ctx,
                       int mode,
                       size_t length,
                       unsigned char iv[16],
                       const unsigned char *input,
