...
/**
 * Note:
 * Hardware offload and HMAC bitmap as follows:
 * bit0  --> 0: MD function only,        1: HMAC function
 * bit16 --> 0: software for MD,         1: hardware offload for MD
 */
#define MBEDTLS_MD_HW               (1 << 16) /* hardware offload for MD  */
#define MBEDTLS_MD_HMAC             (1 << 0)  /* select hmac function */

#define MBEDTLS_MD_HW_MD            (MBEDTLS_MD_HW)
#define MBEDTLS_MD_HW_HMAC          (MBEDTLS_MD_HMAC | MBEDTLS_MD_HW)
#define MBEDTLS_MD_SOFT_MD          (0)
#define MBEDTLS_MD_SOFT_HMAC        (MBEDTLS_MD_HMAC)
 ...
 * \param ctx       Context to set up.
 * \param md_info   Message digest to use.
 * \param mode      select work mode as follows:
 *                  MBEDTLS_MD_HW_MD,   MBEDTLS_MD_HW_HMAC,
 *                  MBEDTLS_MD_SOFT_MD, MBEDTLS_MD_SOFT_HMAC
 *
 * \returns         \c 0 on success,
 *                  \c MBEDTLS_ERR_MD_BAD_INPUT_DATA on parameter failure,
 *                  \c MBEDTLS_ERR_MD_ALLOC_FAILED memory allocation failure.
