...
struct webnet_session* webnet_session_create(int listenfd)
{
    ...
    if (session != RT_NULL)
    {
        ...
        if (tls_srv_session_config(session->tls_session) != RT_EOK)
        {
...
int webnet_session_read(struct webnet_session *session, char *buffer, int length)
{
    ...
    //printf( " webnet_session_read %d bytes:\n%s", read_count, (char *) buffer );
    return read_count;
}
...
void webnet_session_close(struct webnet_session *session)
{
     ...
     * end. Close here */
    #ifdef WEBNET_USING_TLS
    if (tls_srv_session_close(session->tls_session) != RT_EOK)
        return;
    tls_srv_session_destroy(session->tls_session);
    #else
