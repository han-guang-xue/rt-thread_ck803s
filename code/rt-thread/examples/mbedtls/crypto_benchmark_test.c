...
#else
    mbedtls_hardware_poll(NULL, output, len, &use_len);
#endif
    return( 0 );
}
...
__exdata unsigned char buf[BUFSIZE];

typedef struct {
    char md4, md5, ripemd160, sha1, sha256, sha512, hmac,
         arc4, des3, des,
         aes_cbc, aes_gcm, aes_ccm, aes_cmac, des3_cmac,
         sm4_cbc, sm4_gcm, sm4_ccm, sm4_cmac,
         camellia, blowfish,
         havege, ctr_drbg, hmac_drbg,
         rsa, dhm, ecdsa, ecdh;
} todo_list;

int mbedtls_crypto_benchmark_main( int argc, char *argv[] )
{
    int i;
    ...
    else
    {
        ...
        for( i = 1; i < argc; i++ )
        {
            if( strcmp( argv[i], "md4" ) == 0 )
                ...
                todo.sm4_cbc = 1;
            else if( strcmp( argv[i], "sm4_gcm" ) == 0 )
                todo.sm4_gcm = 1;
            else if( strcmp( argv[i], "sm4_ccm" ) == 0 )
                todo.sm4_ccm = 1;
            else if( strcmp( argv[i], "sm4_cmac" ) == 0 )
                todo.sm4_cmac = 1;
    ...
    mbedtls_printf( "\n" );
    mbedtls_printf( "Test Buffer size = 0x%x.\n", BUFSIZE);
    ...
    if( todo.hmac )
    {
        ...
        if( ( md_info = mbedtls_md_info_from_type( MBEDTLS_MD_SHA1 ) ) == NULL )
            mbedtls_exit(1);
        TIME_AND_TSC( "hmac-sha1",
                      mbedtls_md_hmac(md_info, tmp, sizeof(tmp), buf, sizeof(buf), buf) );
#endif

#if defined(MBEDTLS_SHA256_C)
        if( ( md_info = mbedtls_md_info_from_type( MBEDTLS_MD_SHA256 ) ) == NULL )
            mbedtls_exit(1);
        TIME_AND_TSC( "hmac-sha256",
                      mbedtls_md_hmac(md_info, tmp, sizeof(tmp), buf, sizeof(buf), buf) );
#endif
        ...
        if( ( md_info = mbedtls_md_info_from_type( MBEDTLS_MD_SHA512 ) ) == NULL )
            mbedtls_exit(1);
        TIME_AND_TSC( "hmac-sha512",
                      mbedtls_md_hmac(md_info, tmp, sizeof(tmp), buf, sizeof(buf), buf) );
#endif

#if defined(MBEDTLS_SM3_C)
        if( ( md_info = mbedtls_md_info_from_type( MBEDTLS_MD_SM3 ) ) == NULL )
            mbedtls_exit(1);
        TIME_AND_TSC( "hmac-sm3",
                      mbedtls_md_hmac(md_info, tmp, sizeof(tmp), buf, sizeof(buf), buf) );
#endif
    }
    ...
    if( todo.sm4_cbc )
    {
        ...
        {
            mbedtls_snprintf( title, sizeof( title ), "SM4-CBC-%d", keysize );

            memset( buf, 0, sizeof( buf ) );
            memset( tmp, 0, sizeof( tmp ) );
            mbedtls_sm4_setkey_enc( &sm4, tmp, keysize );

            TIME_AND_TSC( title,
                mbedtls_sm4_crypt_cbc( &sm4, MBEDTLS_SM4_ENCRYPT, BUFSIZE, tmp, buf, buf ) );
        }
    ...
    if( todo.sm4_gcm )
    {
        ...
        mbedtls_gcm_init( &gcm );
        for( keysize = 128; keysize <= 128; keysize += 128 )
        {
            mbedtls_snprintf( title, sizeof( title ), "SM4-GCM-%d", keysize );

            memset( buf, 0, sizeof( buf ) );
            memset( tmp, 0, sizeof( tmp ) );
            mbedtls_gcm_setkey( &gcm, MBEDTLS_CIPHER_ID_SM4, tmp, keysize );
    ...
    if( todo.sm4_ccm )
    {
        ...
        for( keysize = 128; keysize <= 128; keysize += 128 )
        {
            mbedtls_snprintf( title, sizeof( title ), "SM4-CCM-%d", keysize );

            memset( buf, 0, sizeof( buf ) );
            memset( tmp, 0, sizeof( tmp ) );
            mbedtls_ccm_setkey( &ccm, MBEDTLS_CIPHER_ID_SM4, tmp, keysize );
    ...
    if( todo.sm4_cmac )
    {
        ...
        for( keysize = 128, cipher_type = MBEDTLS_CIPHER_SM4_128_ECB;
             keysize <= 128;
             keysize += 128, cipher_type += 2 )
        {
            mbedtls_snprintf( title, sizeof( title ), "SM4-CMAC-%d", keysize );
    ...
    if( todo.hmac_drbg )
    {
        ...
        if( mbedtls_hmac_drbg_seed( &hmac_drbg, md_info, myrand, NULL, NULL, 0 ) != 0 )
            mbedtls_exit(1);
        TIME_AND_TSC( "HMAC_DRBG SM3 (NOPR)",
                if( mbedtls_hmac_drbg_random( &hmac_drbg, buf, BUFSIZE ) != 0 )
                mbedtls_exit(1) );
        mbedtls_hmac_drbg_free( &hmac_drbg );
...
MSH_CMD_EXPORT(mbedtls_crypto_benchmark_main, mbedtls crypto benchmark start);
