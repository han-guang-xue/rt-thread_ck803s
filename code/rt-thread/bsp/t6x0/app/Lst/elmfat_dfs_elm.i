# 1 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c"
...
# 1 "<command-line>"
# 1 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c"
# 34 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c"
...
# 35 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2
...
# 36 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2
...
#define AM_DIR 0x10
#define AM_ARC 0x20
# 37 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2
...
# 38 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2
...
# 44 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2
...
# 45 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2

static rt_device_t disk[2] = {0};
...
int dfs_elm_mount(struct dfs_filesystem *fs, unsigned long rwflag, const void *data)
{
    ...
    if (rt_device_control(fs->dev_id, 0x10, &geometry) == 0)
    {
    ...
    if (result == FR_OK)
    {
        ...
        if (dir == (0))
        {
            f_mount((0), (const TCHAR*)logic_nbr, 1);
            disk[index] = (0);
            rt_free(fat);
            return -12;
        }
...
int dfs_elm_unmount(struct dfs_filesystem *fs)
{
    ...
    if (!(fat != (0))) { rt_assert_handler("fat != RT_NULL", __FUNCTION__, 189); };
    ...
    logic_nbr[0] = '0' + index;
    result = f_mount((0), logic_nbr, (BYTE)1);
    if (result != FR_OK)
        return elm_result_to_dfs(result);
...
#define FSM_STATUS_USE_TEMP_DRIVER 1
    ...
    result = f_mkfs(logic_nbr, 0x07, 0, work, 512);
    rt_free(work); work = (0);
    ...
    {
        rt_free(fat);
        f_mount((0), logic_nbr,(BYTE)index);
        disk[index] = (0);
...
int dfs_elm_statfs(struct dfs_filesystem *fs, struct statfs *buf)
{
    ...
    if (!(fs != (0))) { rt_assert_handler("fs != RT_NULL", __FUNCTION__, 307); };
    if (!(buf != (0))) { rt_assert_handler("buf != RT_NULL", __FUNCTION__, 308); };
...
int dfs_elm_open(struct dfs_fd *file)
{
    ...
    else
    {
        ...
        {
            file->pos = fd->fptr;
            file->size = ((fd)->obj.objsize);
            file->data = fd;

            if (file->flags & 02000)
            {

                f_lseek(fd, ((fd)->obj.objsize));
                file->pos = fd->fptr;
            }
        }
        else
...
int dfs_elm_close(struct dfs_fd *file)
{
    ...
    if (file->type == 2)
    {
        ...
        dir = (DIR *)(file->data);
        if (!(dir != (0))) { rt_assert_handler("dir != RT_NULL", __FUNCTION__, 465); };
    ...
    else if (file->type == 0)
    {
        FIL *fd;
        fd = (FIL *)(file->data);
        if (!(fd != (0))) { rt_assert_handler("fd != RT_NULL", __FUNCTION__, 474); };
...
int __attribute__((section(".fast"))) dfs_elm_read(struct dfs_fd *file, void *buf, size_t len)
{
    FIL *fd;
    FRESULT result;
    UINT byte_read;
    ...
    fd = (FIL *)(file->data);
    if (!(fd != (0))) { rt_assert_handler("fd != RT_NULL", __FUNCTION__, 504); };
...
int __attribute__((section(".fast"))) dfs_elm_write(struct dfs_fd *file, const void *buf, size_t len)
{
    FIL *fd;
    FRESULT result;
    UINT byte_write;
    ...
    fd = (FIL *)(file->data);
    if (!(fd != (0))) { rt_assert_handler("fd != RT_NULL", __FUNCTION__, 527); };
...
int dfs_elm_flush(struct dfs_fd *file)
{
    ...
    fd = (FIL *)(file->data);
    if (!(fd != (0))) { rt_assert_handler("fd != RT_NULL", __FUNCTION__, 545); };
...
int dfs_elm_lseek(struct dfs_fd *file, off_t offset)
{
    ...
    if (file->type == 0)
    {
        ...
        fd = (FIL *)(file->data);
        if (!(fd != (0))) { rt_assert_handler("fd != RT_NULL", __FUNCTION__, 560); };
    ...
    else if (file->type == 2)
    {
        ...
        dir = (DIR *)(file->data);
        if (!(dir != (0))) { rt_assert_handler("dir != RT_NULL", __FUNCTION__, 576); };
...
int dfs_elm_getdents(struct dfs_fd *file, struct dirent *dirp, uint32_t count)
{
    ...
    dir = (DIR *)(file->data);
    if (!(dir != (0))) { rt_assert_handler("dir != RT_NULL", __FUNCTION__, 599); };
...
int dfs_elm_stat(struct dfs_filesystem *fs, const char *path, struct stat *st)
{
    ...
    if (result == FR_OK)
    {
        ...
        st->st_mode = 0100000 | 00400 | 00040 | 00004 |
                      00200 | 00020 | 00002;
        if (file_info.fattrib & 0x10)
        {
            st->st_mode &= ~0100000;
            st->st_mode |= 0040000 | 00100 | 00010 | 00001;
        }
        if (file_info.fattrib & 0x01)
            st->st_mode &= ~(00200 | 00020 | 00002);
...
const init_fn_t __rt_init_elm_init __attribute__((section(".rti_fn.""4"))) = elm_init;

int mount_elm_ata(int argc, char **argv)
{
    const char* path;

    if (argc != 2)
    {
        rt_kprintf("Usage: mount ata to a path\n");
        rt_kprintf("Like: mt_elm_ata /elm\n");
        return 0;
    }

    path = argv[1];

    if (dfs_mount("ata", path, "elm", 0, 0)== 0)
    {
...
# 855 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2
const char __fsym___cmd_mt_elm_ata_name[] __attribute__((section(".rodata.name"))) = "__cmd_mt_elm_ata"; const char __fsym___cmd_mt_elm_ata_desc[] __attribute__((section(".rodata.name"))) = "mount ata(fat) to a path"; const struct finsh_syscall __fsym___cmd_mt_elm_ata __attribute__((section("FSymTab")))= { __fsym___cmd_mt_elm_ata_name, __fsym___cmd_mt_elm_ata_desc, (syscall_func)&mount_elm_ata };;
...
# 862 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/filesystems/elmfat/dfs_elm.c" 2
...
DRESULT __attribute__((section(".fast"))) disk_read (BYTE drv, BYTE* buff, DWORD sector, UINT count)
{
    rt_size_t result;
...
DRESULT __attribute__((section(".fast"))) disk_write (BYTE drv, const BYTE* buff, DWORD sector, UINT count)
{
    rt_size_t result;
    rt_device_t device = disk[drv];
...
DRESULT __attribute__((section(".fast"))) disk_ioctl(BYTE drv, BYTE ctrl, void *buff)
{
    rt_device_t device = disk[drv];
    ...
    if (ctrl == 1)
    {
        ...
        rt_device_control(device, 0x10, &geometry);
    ...
    {
        rt_device_control(device, 0x11, (0));
    }
    else if (ctrl == 4)
    {
        rt_device_control(device, 0x12, buff);
    }
...
int ff_cre_syncobj(BYTE drv, rt_mutex_t *m)
{
    char name[8];
    rt_mutex_t mutex;

    rt_snprintf(name, sizeof(name), "fat%d", drv);
    mutex = rt_mutex_create(name, 0x00);
    if (mutex != (0))
    {
        *m = mutex;
        return 1;
...
int ff_del_syncobj(rt_mutex_t m)
{
    if (m != (0))
        rt_mutex_delete(m);
...
int __attribute__((section(".fast"))) ff_req_grant(rt_mutex_t m)
{
    if (rt_mutex_take(m, 1000) == 0)
        return 1;

    return 0;
}

void __attribute__((section(".fast"))) ff_rel_grant(rt_mutex_t m)
{
    rt_mutex_release(m);
}
