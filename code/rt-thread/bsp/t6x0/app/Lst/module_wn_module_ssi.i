# 1 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
# 1 "E:\\YCXGIT\\T6x0_RTOS_SDK_V0.9.3_b1\\T6x0_RTOS_SDK_V0.9.3\\code\\rt-thread\\bsp\\t6x0\\app//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
# 23 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
# 1 "../../../packages/webnet-2.0.0/inc/webnet.h" 1
# 48 "../../../packages/webnet-2.0.0/inc/webnet.h"
...
# 49 "../../../packages/webnet-2.0.0/inc/webnet.h" 2
...
# 24 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 2
...
# 25 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 2
...
# 26 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 2
...
# 29 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 2
# 40 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
...
# 50 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 3 4
                              2
# 50 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
                                      );

    lseek(fd, 0, 
# 52 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 3 4
                0
# 52 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
                        );
    if (file_length <= 0)
    {
        close(fd);
...
static void _webnet_ssi_dofile(struct webnet_session* session, int fd)
{
    ...
    length = lseek(fd, 0, 
# 93 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 3 4
                         2
# 93 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
                                 );
    lseek(fd, 0, 
# 94 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c" 3 4
                0
# 94 "../../../packages/webnet-2.0.0/module/wn_module_ssi.c"
                        );


    path = (char*) rt_malloc(256);
    ...
    while (offset < end)
    {
        ...
        ssi_begin = strstr(offset, "<!--#include ");
        if (ssi_begin == (0))
        {
        ...
        else
        {
            ...
            offset = ssi_begin + sizeof("<!--#include ") - 1;
            include_begin = strstr(ssi_begin, "virtual=\"");
            if (include_begin != (0))
            {
                filename = include_begin + sizeof("virtual=\"") - 1;
            ...
            {
                include_begin = strstr(ssi_begin, "file=\"");
                if (include_begin != (0))
                {
                    filename = include_begin + sizeof("file=\"") - 1;
                    include_end = strstr(filename, "\"");
                    *include_end = '\0';
            ...
            offset = ssi_end + sizeof("\" -->") - 1;
        }
    }


__exit:
    if (path != (0)) rt_free(path);
    if (buffer != (0)) rt_free(buffer);
}
...
int webnet_module_ssi(struct webnet_session* session, int event)
{
    ...
    if (!(session != (0))) { rt_assert_handler("session != RT_NULL", __FUNCTION__, 198); };
    request = session->request;
    if (!(request != (0))) { rt_assert_handler("request != RT_NULL", __FUNCTION__, 200); };
    ...
    if (event == (1 << 2))
    {
        ...
        index = 0;
        while (ssi_extension[index] != (0))
        {
            if (strstr(request->path, ssi_extension[index]) != (0))
            {
...

        {
            ...
            ssi_filename = (char*) rt_malloc (256);
            if (ssi_filename != (0))
            {
                rt_snprintf(ssi_filename, 256, "%s/index.shtm", request->path);
                fd = open(ssi_filename, 00, 0);

                if (fd >= 0)
                {
                    rt_free(ssi_filename);
                    _webnet_ssi_dofile(session, fd);
                    close(fd);
                    return 1;
                }
