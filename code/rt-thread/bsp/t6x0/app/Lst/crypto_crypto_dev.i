# 1 "../../../components/drivers/crypto/crypto_dev.c"
...
void xmem_init(void);
# 2 "../../../components/drivers/crypto/crypto_dev.c" 2
...
static rt_device_t crypto_dev = (0);
...
static rt_err_t rt_crypto_init(rt_device_t dev)
{
    ...
    rt_mutex_init(&(crypto->lock_cipher), "lock_cipher", 0x00);
    rt_mutex_init(&(crypto->lock_pk), "lock_pk", 0x00);
    rt_mutex_init(&(crypto->lock_hash), "lock_hash", 0x00);
    rt_mutex_init(&(crypto->lock_rng), "lock_rng", 0x00);
...
static rt_err_t rt_crypto_ioctl(rt_device_t dev, int cmd, void *args)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 79); };
...
# 102 "../../../components/drivers/crypto/crypto_dev.c"
rt_err_t rt_crypto_register(rt_crypto_device_t *crypto,
                               const char *name,
                               rt_uint32_t flag,
                               void *data)
{
    ...
    if (!(crypto != (0))) { rt_assert_handler("crypto != RT_NULL", __FUNCTION__, 110); };
