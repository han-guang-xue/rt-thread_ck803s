# 1 "../../../packages/webclient-2.0.1/src/webclient.c"
# 1 "E:\\YCXGIT\\T6x0_RTOS_SDK_V0.9.3_b1\\T6x0_RTOS_SDK_V0.9.3\\code\\rt-thread\\bsp\\t6x0\\app//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "../../../packages/webclient-2.0.1/src/webclient.c"
# 17 "../../../packages/webclient-2.0.1/src/webclient.c"
...
# 18 "../../../packages/webclient-2.0.1/src/webclient.c" 2
...
# 21 "../../../packages/webclient-2.0.1/src/webclient.c" 2



# 1 "../../../components/dfs/filesystems/net/netdb.h" 1
# 32 "../../../components/dfs/filesystems/net/netdb.h"
# 1 "../../../components/net/lwip-2.0.2/src/include/lwip/netdb.h" 1
# 37 "../../../components/net/lwip-2.0.2/src/include/lwip/netdb.h"
...
int getaddrinfo(const char *nodename,
       ...
       const struct addrinfo *hints,
       struct addrinfo **res);
# 25 "../../../packages/webclient-2.0.1/src/webclient.c" 2
# 1 "../../../components/dfs/filesystems/net/socket/sys/socket.h" 1
# 34 "../../../components/dfs/filesystems/net/socket/sys/socket.h"
...
# 26 "../../../packages/webclient-2.0.1/src/webclient.c" 2
# 39 "../../../packages/webclient-2.0.1/src/webclient.c"
# 1 "../../../include/rtdbg.h" 1
# 40 "../../../packages/webclient-2.0.1/src/webclient.c" 2
...
static int webclient_send(struct webclient_session* session, const unsigned char *buffer, size_t len, int flag)
{
    ...
    {
        return tls_cli_session_write(session->tls_session, buffer, len);
    }
...
static int webclient_recv(struct webclient_session* session, unsigned char *buffer, size_t len, int flag)
{
    ...
    if(session->tls_session)
    {
        return tls_cli_session_read(session->tls_session, buffer, len);
    }
...
static int webclient_read_line(struct webclient_session *session, char *buffer, int size)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 75); };
    if (!(buffer)) { rt_assert_handler("buffer", __FUNCTION__, 76); };
    ...
    while (count < size)
    {
        ...
        if(session->tls_session && rc == -0x6900)
            continue;
    ...
    if (count > size)
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "read line failed. The line data length is out of buffer size(%d)!", count); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        return -WEBCLIENT_ERROR;
    }

    return count;
}
# 122 "../../../packages/webclient-2.0.1/src/webclient.c"
                                     ...
                                     const char *url, char **request)
{
    ...
    if (!(res)) { rt_assert_handler("res", __FUNCTION__, 132); };
    if (!(request)) { rt_assert_handler("request", __FUNCTION__, 133); };
    ...
    {
        char *host_addr_new = rt_malloc(host_addr_len + 1);
        ...
        if(session->tls_session)
        {
            if (session->tls_session->host != (0))
                rt_free(session->tls_session->host);
            session->tls_session->host = rt_strdup(host_addr_new);
        }

    }

    if ((0) >= 1) { switch(0) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "host address: %s , port: %s", session->host, port_str); rt_kprintf("\033[""0""m"); }; if ((0) >= 1) { rt_kprintf("\n"); };
...

    {
        ...
        if(session->tls_session)
        {
            ...
            if (session->tls_session->port != (0))
                rt_free(session->tls_session->port);
            session->tls_session->port = rt_strdup(port_str);
            ret = getaddrinfo(session->tls_session->host, port_str, &hint, res);
            if (ret != 0)
            {
                if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "getaddrinfo err: %d '%s'", ret, session->host); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
                rc = -WEBCLIENT_ERROR;
            }
...
# 302 "../../../packages/webclient-2.0.1/src/webclient.c"
static int webclient_open_tls(struct webclient_session *session, const char *URI)
{
    int tls_ret = 0;

    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 306); };

    session->tls_session = tls_cli_session_create("TMP_HOST", "TMP_PORT", 4096);
    if (session->tls_session == (0))
    {
        return -WEBCLIENT_NOMEM;
    }

    if((tls_ret = tls_cli_session_init(session->tls_session, 0)) < 0)
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "initialize https client failed return: -0x%x.", -tls_ret); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        return -WEBCLIENT_ERROR;
    }

    return WEBCLIENT_OK;
}
# 333 "../../../packages/webclient-2.0.1/src/webclient.c"
static int webclient_connect(struct webclient_session *session, const char *URI)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 341); };
    if (!(URI)) { rt_assert_handler("URI", __FUNCTION__, 342); };
    ...
    if (strncmp(URI, "https://", 8) == 0)
    {
        ...
        {
            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "connect failed, https client open URI(%s) failed!", URI); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
            return -WEBCLIENT_ERROR;
        }
    ...
    if (rc != WEBCLIENT_OK)
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "connect failed, resolve address error(%d).", rc); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        goto __exit;
    }
    ...
    if (*req_url)
    {
        session->req_url = rt_strdup(req_url);
    }
    ...
    if (session->tls_session)
    {
        ...
        if ((tls_ret = tls_cli_session_config(session->tls_session)) < 0)
        {
            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "connect failed, https client context return: -0x%x", -tls_ret); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
            return -WEBCLIENT_ERROR;
        }

        freeaddrinfo(res);

        if ((tls_ret = tls_cli_session_connect(session->tls_session)) < 0)
        {
            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "connect failed, https client connect return: -0x%x", -tls_ret); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
            rc = -WEBCLIENT_CONNECT_FAILED;
            if (tls_ret == -0x0044)
            {
                session->tls_session->srv_fd.fd = -1;
            }
  ...
  getaddrinfo(session->tls_session->host, session->tls_session->port, &hint, &res);

        socket_handle = session->tls_session->srv_fd.fd;


        setsockopt(socket_handle, 0xfff, 0x1006, (void*) &timeout,
                sizeof(timeout));
        setsockopt(socket_handle, 0xfff, 0x1005, (void*) &timeout,
                sizeof(timeout));
    ...
    {
        socket_handle = socket(res->ai_family, 1, 6);
        if (socket_handle < 0)
        {
            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "connect failed, create socket(%d) error.", socket_handle); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        ...
        if (connect(socket_handle, res->ai_addr, res->ai_addrlen) != 0)
        {
            ...
            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "connect failed, connect socket(%d) error.", socket_handle); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
            rc = -WEBCLIENT_CONNECT_FAILED;
            goto __exit;
        }
...
# 472 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_header_fields_add(struct webclient_session *session, const char *fmt, ...)
{
    rt_int32_t length;
    va_list args;

    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 477); };
    if (!(session->header->buffer)) { rt_assert_handler("session->header->buffer", __FUNCTION__, 478); };

    
# 480 "../../../packages/webclient-2.0.1/src/webclient.c" 3 4
   __builtin_va_start(
# 480 "../../../packages/webclient-2.0.1/src/webclient.c"
   args
# 480 "../../../packages/webclient-2.0.1/src/webclient.c" 3 4
   ,
# 480 "../../../packages/webclient-2.0.1/src/webclient.c"
...
# 480 "../../../packages/webclient-2.0.1/src/webclient.c"
    ...
    if (length < 0)
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "add fields header data failed, return length(%d) error.", length); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        return -WEBCLIENT_ERROR;
    }
    
# 488 "../../../packages/webclient-2.0.1/src/webclient.c" 3 4
   __builtin_va_end(
# 488 "../../../packages/webclient-2.0.1/src/webclient.c"
   args
# 488 "../../../packages/webclient-2.0.1/src/webclient.c" 3 4
   )
# 488 "../../../packages/webclient-2.0.1/src/webclient.c"
    ...
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "not enough header buffer size(%d)!", session->header->size); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        return -WEBCLIENT_ERROR;
    }

    return length;
}
# 511 "../../../packages/webclient-2.0.1/src/webclient.c"
const char *webclient_header_fields_get(struct webclient_session *session, const char *fields)
{
    char *resp_buf = (0);
    size_t resp_buf_len = 0;

    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 516); };
    if (!(session->header->buffer)) { rt_assert_handler("session->header->buffer", __FUNCTION__, 517); };
...
# 556 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_resp_status_get(struct webclient_session *session)
{
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 558); };

    return session->resp_status;
}
# 570 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_content_length_get(struct webclient_session *session)
{
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 572); };
...
static int webclient_send_header(struct webclient_session *session, int method)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 582); };
    ...
    else
    {
        if (method != WEBCLIENT_USER_METHOD)
        {
            ...
            if (memcmp(header, "HTTP/1.", rt_strlen("HTTP/1.")))
            {
                ...
                header_buffer = rt_strdup(session->header->buffer);
                if (header_buffer == (0))
                {
                    if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "no memory for header buffer!"); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
                    rc = -WEBCLIENT_NOMEM;
                    goto __exit;
                }
            ...
            if (session->header->length > session->header->size)
            {
                if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "send header failed, not enough header buffer size(%d)!", session->header->size); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
...
# 678 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_handle_response(struct webclient_session *session)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 686); };
    ...
    mime_ptr = rt_strdup(session->header->buffer);
    if (mime_ptr == (0))
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "no memory for get http status code buffer!"); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        return -WEBCLIENT_NOMEM;
    }
    ...
    if (rt_strstr(mime_ptr, "HTTP/1."))
    {
        ...
        session->resp_status = (int) strtol(ptr, (0), 10);
    }


    if (webclient_header_fields_get(session, "Content-Length") != (0))
    {
    ...
    if (transfer_encoding && rt_strcmp(transfer_encoding, "chunked") == 0)
    {
        ...
        session->chunk_sz = strtol(line, (0), 16);
...
# 786 "../../../packages/webclient-2.0.1/src/webclient.c"
struct webclient_session *webclient_session_create(size_t header_sz)
{
    ...
    if (session == (0))
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "webclient create failed, no memory for webclient session!"); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        return (0);
    }

    session->content_length = -1;

    session->header = (struct webclient_header *) rt_calloc(1, sizeof(struct webclient_header));
    if (session->header == (0))
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "webclient create failed, no memory for session header!"); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        rt_free(session);
        session = (0);
        return (0);
    }
    ...
    session->header->buffer = (char *) rt_malloc(header_sz);
    if (session->header->buffer == (0))
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "webclient create failed, no memory for session header buffer!"); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        rt_free(session->header);
        rt_free(session);
        session = (0);
        return (0);
    }
...
# 835 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_get(struct webclient_session *session, const char *URI)
{
    int rc = WEBCLIENT_OK;
    int resp_status = 0;

    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 840); };
    if (!(URI)) { rt_assert_handler("URI", __FUNCTION__, 841); };
    ...
    if (resp_status > 0)
    {
        ...
        {
            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "get failed, handle response(%d) error!", resp_status); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
            return resp_status;
        }
    }
...
# 890 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_get_position(struct webclient_session *session, const char *URI, int position)
{
    int rc = WEBCLIENT_OK;
    int resp_status = 0;

    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 895); };
    if (!(URI)) { rt_assert_handler("URI", __FUNCTION__, 896); };
...
# 954 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_post(struct webclient_session *session, const char *URI, const char *post_data)
{
    int rc = WEBCLIENT_OK;
    int resp_status = 0;

    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 959); };
    if (!(URI)) { rt_assert_handler("URI", __FUNCTION__, 960); };
    ...
    if (post_data)
    {
        ...
        if (resp_status != 200)
        {
            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "post failed, handle response(%d) error.", resp_status); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
            return resp_status;
        }
    }
...
# 1001 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_set_timeout(struct webclient_session *session, int millisecond)
{
    struct timeval timeout;
    int second = rt_tick_from_millisecond(millisecond) / 1000;

    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 1006); };

    timeout.tv_sec = second;
    timeout.tv_usec = 0;


    setsockopt(session->socket, 0xfff, 0x1006,
               (void *) &timeout, sizeof(timeout));
    setsockopt(session->socket, 0xfff, 0x1005,
               (void *) &timeout, sizeof(timeout));
...
static int webclient_next_chunk(struct webclient_session *session)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 1025); };
...
# 1082 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_read(struct webclient_session *session, unsigned char *buffer, size_t length)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 1088); };
    ...
    do
    {
        ...
        if (bytes_read <= 0)
        {
            ...
            if(session->tls_session && bytes_read == -0x6900)
                continue;

            if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "receive data error(%d).", bytes_read); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
            ...
            else
            {
                if (*_rt_errno() == 11 || *_rt_errno() == 11)
                {
                    ...
                    if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "receive data timeout."); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
                    return -WEBCLIENT_TIMEOUT;
                }
                else
                {
...
# 1213 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_write(struct webclient_session *session, const unsigned char *buffer, size_t length)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 1219); };
    ...
    do
    {
        ...
        if (bytes_write <= 0)
        {
            ...
            if(session->tls_session && bytes_write == -0x6880)
                continue;
...
# 1283 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_close(struct webclient_session *session)
{
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 1285); };


    if (session->tls_session)
    {
        tls_cli_session_close(session->tls_session);
        tls_cli_session_destroy(session->tls_session);
    }
    else
    {
        if (session->socket >= 0)
...
# 1309 "../../../packages/webclient-2.0.1/src/webclient.c"
    if (session->host)
    {
        rt_free(session->host);
    }
    ...
    if (session->req_url)
    {
        rt_free(session->req_url);
    }

    if (session->header && session->header->buffer)
    {
        rt_free(session->header->buffer);
    }
...
# 1346 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_response(struct webclient_session *session, unsigned char **response)
{
    ...
    if (!(session)) { rt_assert_handler("session", __FUNCTION__, 1352); };
    if (!(response)) { rt_assert_handler("response", __FUNCTION__, 1353); };
    ...
    *response = (0);
    ...
    if (session->content_length < 0)
    {
        ...
        while (1)
        {
            unsigned char *new_resp = (0);

            result_sz = total_read + 4096;
            new_resp = rt_realloc(response_buf, result_sz + 1);
            if (new_resp == (0))
            {
                if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "no memory for realloc new response buffer!"); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
                break;
            }
...
# 1440 "../../../packages/webclient-2.0.1/src/webclient.c"
int webclient_request(const char *URI, const char *header, const char *post_data, unsigned char **response)
{
    struct webclient_session *session;
    int rc = WEBCLIENT_OK;
    ...
    if (!(URI)) { rt_assert_handler("URI", __FUNCTION__, 1446); };

    if (post_data == (0) && response == (0))
    {
        if ((3) >= 1) { switch(3) { case 3: rt_kprintf("\033[""31""m"); break; case 2: rt_kprintf("\033[""33""m"); break; case 1: rt_kprintf("\033[""32""m"); break; default: break; } rt_kprintf("web" "request get failed, get response data cannot be empty."); rt_kprintf("\033[""0""m"); }; if ((3) >= 1) { rt_kprintf("\n"); };
        return -WEBCLIENT_ERROR;
    }
