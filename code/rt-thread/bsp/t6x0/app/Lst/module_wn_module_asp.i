# 1 "../../../packages/webnet-2.0.0/module/wn_module_asp.c"
# 1 "E:\\YCXGIT\\T6x0_RTOS_SDK_V0.9.3_b1\\T6x0_RTOS_SDK_V0.9.3\\code\\rt-thread\\bsp\\t6x0\\app//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "../../../packages/webnet-2.0.0/module/wn_module_asp.c"
# 24 "../../../packages/webnet-2.0.0/module/wn_module_asp.c"
...
# 25 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 2
...
# 29 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 2
...
# 40 "../../../components/net/lwip-2.0.2/src/include/lwip/netif.h"
...
# 41 "../../../components/net/lwip-2.0.2/src/include/lwip/netif.h" 2
...
# 35 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 2
...
# 38 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 2
...
# 39 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 2
...
# 40 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 2
...
static struct webnet_asp_variable* _webnet_asp_vars = (0);
rt_uint32_t _webnet_asp_vars_count = 0;

void webnet_asp_add_var(const char* name, void (*handler)(struct webnet_session* session))
{
    if (_webnet_asp_vars == (0))
    {
        _webnet_asp_vars_count = 1;
        _webnet_asp_vars = (struct webnet_asp_variable*)rt_malloc (sizeof(struct webnet_asp_variable) *
                           _webnet_asp_vars_count);
    }
    else
    {
        _webnet_asp_vars_count += 1;
        _webnet_asp_vars = (struct webnet_asp_variable*) rt_realloc (_webnet_asp_vars, sizeof(struct webnet_asp_variable) *
                           _webnet_asp_vars_count);
    }

    if (!(_webnet_asp_vars != (0))) { rt_assert_handler("_webnet_asp_vars != RT_NULL", __FUNCTION__, 66); };

    _webnet_asp_vars[_webnet_asp_vars_count - 1].name = rt_strdup(name);
...
static void _default_asp_handler(struct webnet_session* session, const char* name)
{
    ...
    {
        webnet_session_printf(session, "%s", ip4addr_ntoa((const ip4_addr_t*)&(session->cliaddr.sin_addr)));
    }
    else if (strncmp(name, "REMOTE_PORT", 11) == 0)
    {
        webnet_session_printf(session, "%d", lwip_htons(session->cliaddr.sin_port));
    }
    else if (strncmp(name, "SERVER_ADDR", 11) == 0)
    {

        webnet_session_printf(session, "%s", ip4addr_ntoa((const ip4_addr_t*)&((netif_default->ip_addr))));


    }
    else if (strncmp(name, "SERVER_PORT", 11) == 0)
    {
        webnet_session_printf(session, "%d", 443);
    }
...
static void _webnet_asp_dofile(struct webnet_session* session, int fd)
{
    ...
    length = lseek(fd, 0, 
# 155 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 3 4
                         2
# 155 "../../../packages/webnet-2.0.0/module/wn_module_asp.c"
                                 );
    lseek(fd, 0, 
# 156 "../../../packages/webnet-2.0.0/module/wn_module_asp.c" 3 4
                0
# 156 "../../../packages/webnet-2.0.0/module/wn_module_asp.c"
                        );
...
int webnet_module_asp(struct webnet_session* session, int event)
{
    ...
    if (!(session != (0))) { rt_assert_handler("session != RT_NULL", __FUNCTION__, 234); };
    request = session->request;
    if (!(request != (0))) { rt_assert_handler("request != RT_NULL", __FUNCTION__, 236); };
    ...
    if (event == (1 << 2))
    {
        ...
        if ((strstr(request->path, ".asp") != (0)) ||
                (strstr(request->path, ".ASP") != (0)))
        {
        ...
        else
        {
            ...
            asp_filename = (char*) rt_malloc (256);
            if (asp_filename != (0))
            {
                rt_snprintf(asp_filename, 256, "%s/index.asp", request->path);
                fd = open(asp_filename, 00, 0);

                if (fd >= 0)
                {
                    rt_free(asp_filename);
                    _webnet_asp_dofile(session, fd);
                    close(fd);
                    return 1;
                }
