...
# 4 "../drivers/drv_spi.c" 2
# 1 "../libraries/inc/tih/spi.h" 1
# 42 "../libraries/inc/tih/spi.h"
...
void spi_intr_enable(unsigned int intr_src);
void spi_intr_disable(unsigned int intr_src);
unsigned int spi_intr_status(void);
void spi_intr_clear(unsigned int intr_src);
void spi_frame_output_port_set(unsigned int port_num);
void spi_frame_output_low(void);
void spi_frame_output_high(void);
unsigned int spi_recv(unsigned char *buf, unsigned int nbytes);
unsigned int spi_send(unsigned char *buf, unsigned int nbytes);
int spi_hw_init(spi_config_t *config);
void spi_send_recv(unsigned char *send_buff,unsigned char *recv_buff,unsigned char nbytes);
# 5 "../drivers/drv_spi.c" 2



extern unsigned char wsize_in_bytes;
...
static rt_err_t drv_spi_configure(struct rt_spi_device* device, struct rt_spi_configuration* configuration)
{
    ...
    SPI_APB_CLOCK = clock_freq_get(MODULE_SPI_IO);
...
static rt_uint32_t __attribute__((section(".fast"))) drv_spi_xfer(struct rt_spi_device *device, struct rt_spi_message *message)
{
    if (!(device != (0))) { rt_assert_handler("device != RT_NULL", __FUNCTION__, 74); };
    if (!(device->bus != (0))) { rt_assert_handler("device->bus != RT_NULL", __FUNCTION__, 75); };

    if(message->cs_take) {
        spi_frame_output_low();
    }
    spi_send_recv((unsigned char *)message->send_buf, (unsigned char *)message->recv_buf, message->length);
...
const init_fn_t __rt_init_drv_spi_init __attribute__((section(".rti_fn.""1"))) = drv_spi_init;
