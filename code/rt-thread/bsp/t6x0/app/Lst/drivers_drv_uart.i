# 1 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/bsp/t6x0/drivers/drv_uart.c"
...
void xmem_init(void);
# 2 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/bsp/t6x0/drivers/drv_uart.c" 2
...
# 3 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/bsp/t6x0/drivers/drv_uart.c" 2
...
# 4 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/bsp/t6x0/drivers/drv_uart.c" 2
...
static unsigned int serial_index[1] = {0};


void __attribute__((section(".fast"))) uart0_irq_handler(void)
{
    unsigned int status;

    status = uart_intr_status(0);
    uart_intr_clear(0, status);
    rt_hw_interrupt_clear(14);

    rt_hw_serial_isr(&serial[0],0x01);
}
# 41 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/bsp/t6x0/drivers/drv_uart.c"
static rt_err_t __attribute__((section(".fast"))) drv_uart_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    unsigned int uart;
    unsigned int parity;
    uart_config_t config;

    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 47); };
    uart = *(unsigned int *)serial->parent.user_data;
    if (!((uart == 0) || (uart == 1))) { rt_assert_handler("(uart == UART0) || (uart == UART1)", __FUNCTION__, 49); };
...
static rt_err_t __attribute__((section(".fast"))) drv_uart_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    unsigned int uart;

    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 73); };
    uart = *(unsigned int *)serial->parent.user_data;
    if (!((uart == 0) || (uart == 1))) { rt_assert_handler("(uart == UART0) || (uart == UART1)", __FUNCTION__, 75); };

    switch (cmd)
    {
    case 0x11:

        uart_intr_disable(uart, (1 << 2));
        break;

    case 0x10:

        if (uart == 0) {
            uart_intr_enable(uart, (1 << 2));
            rt_hw_interrupt_install(14, (rt_isr_handler_t)uart0_irq_handler, (void *)0, "UART0");
...
static int __attribute__((section(".fast"))) drv_uart_putc(struct rt_serial_device *serial, char c)
{
    unsigned int uart;

    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 108); };
    uart = *(unsigned int *)serial->parent.user_data;
    if (!((uart == 0) || (uart == 1))) { rt_assert_handler("(uart == UART0) || (uart == UART1)", __FUNCTION__, 110); };
...
static int __attribute__((section(".fast"))) uart_getc_no_poll(unsigned int uart, unsigned char *ch)
{
...
static int __attribute__((section(".fast"))) drv_uart_getc(struct rt_serial_device *serial)
{
    uint8_t ch;
    unsigned int uart;

    if (!(serial != (0))) { rt_assert_handler("serial != RT_NULL", __FUNCTION__, 133); };
    uart = *(unsigned int *)serial->parent.user_data;
    if (!((uart == 0) || (uart == 1))) { rt_assert_handler("(uart == UART0) || (uart == UART1)", __FUNCTION__, 135); };
...
static int drv_uart_init(void)
{
    ...
    rt_hw_serial_register(&serial[0],"uart0", 0x003 | 0x100, &serial_index[0]);
...
const init_fn_t __rt_init_drv_uart_init __attribute__((section(".rti_fn.""1"))) = drv_uart_init;
