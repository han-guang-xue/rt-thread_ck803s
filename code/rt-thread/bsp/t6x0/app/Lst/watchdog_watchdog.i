...
# 29 "../../../components/drivers/include/drivers/watchdog.h" 2
...
rt_err_t rt_hw_watchdog_register(rt_watchdog_t *wdt,
                                 ...
                                 rt_uint32_t flag,
                                 void *data);
# 27 "../../../components/drivers/watchdog/watchdog.c" 2
...
static rt_err_t rt_watchdog_init(struct rt_device *dev)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 37); };
...
static rt_err_t rt_watchdog_close(struct rt_device *dev)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 56); };
    wtd = (rt_watchdog_t *)dev;

    if (wtd->ops->control(wtd, (6), (0)) != 0)
    {
                                    ...
                                    void *args)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 75); };
    wtd = (rt_watchdog_t *)dev;

    return (wtd->ops->control(wtd, cmd, args));
}
# 96 "../../../components/drivers/watchdog/watchdog.c"
                                 ...
                                 void *data)
{
    ...
    if (!(wtd != (0))) { rt_assert_handler("wtd != RT_NULL", __FUNCTION__, 102); };
