# 1 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/src/memheap.c"
...
# 1 "<command-line>"
# 1 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/src/memheap.c"
# 32 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/src/memheap.c"
# 1 "../../../include/rthw.h" 1
# 30 "../../../include/rthw.h"
#define __RT_HW_H__ 
...
void xmem_init(void);
# 33 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/src/memheap.c" 2
...
# 60 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/src/memheap.c"
rt_err_t rt_memheap_init(struct rt_memheap *memheap,
                         const char *name,
                         void *start_addr,
                         rt_uint32_t size)
{
    ...
    if (!(memheap != (0))) { rt_assert_handler("memheap != RT_NULL", __FUNCTION__, 67); };


    rt_object_init(&(memheap->parent), RT_Object_Class_MemHeap, name);

    memheap->start_addr = start_addr;
    memheap->pool_size = ((size) & ~((8) - 1));
    memheap->available_size = memheap->pool_size - (2 * (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
    memheap->max_used_size = memheap->pool_size - memheap->available_size;


    item = &(memheap->free_header);
    item->magic = 0x1ea01ea0;
    ...
    item->next = (struct rt_memheap_item *)
                 ((rt_uint8_t *)item + memheap->available_size + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
    item->prev = item->next;
    ...
    rt_sem_init(&(memheap->lock), name, 1, 0x00);

    do { if (0) rt_kprintf ("memory heap: start addr 0x%08x, size %d, free list header 0x%08x\n", start_addr, size, &(memheap->free_header)); } while (0)
...
rt_err_t __attribute__((section(".fast"))) rt_memheap_detach(struct rt_memheap *heap)
{
    if (!(heap)) { rt_assert_handler("heap", __FUNCTION__, 136); };
...
void __attribute__((section(".fast"))) *rt_memheap_alloc(struct rt_memheap *heap, rt_uint32_t size)
{
    rt_err_t result;
    rt_uint32_t free_size;
    struct rt_memheap_item *header_ptr;

    if (!(heap != (0))) { rt_assert_handler("heap != RT_NULL", __FUNCTION__, 152); };
    ...
    do { if (0) rt_kprintf ("allocate %d on heap:%8.*s", size, 8, heap->parent.name); } while (0)
                                                                          ;
    ...
    if (size < heap->available_size)
    {
        ...
        result = rt_sem_take(&(heap->lock), -1);
        if (result != 0)
        {
            rt_set_errno(result);
        ...
        while (header_ptr != heap->free_list && free_size < size)
        {
            ...
            free_size = ((rt_uint32_t)header_ptr->next - (rt_uint32_t)header_ptr - (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
            if (free_size < size)
            {
        ...
        if (free_size >= size)
        {
            ...
            if (free_size >= (size + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)) + 12))
            {
                struct rt_memheap_item *new_ptr;


                new_ptr = (struct rt_memheap_item *)
                          (((rt_uint8_t *)header_ptr) + size + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));

                do { if (0) rt_kprintf ("split: block[0x%08x] nextm[0x%08x] prevm[0x%08x] to new[0x%08x]\n", header_ptr, header_ptr->next, header_ptr->prev, new_ptr); } while (0)
                ...
                do { if (0) rt_kprintf ("new ptr: next_free 0x%08x, prev_free 0x%08x\n", new_ptr->next_free, new_ptr->prev_free); } while (0)
                ...
                heap->available_size = heap->available_size -
                                       size -
                                       (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1));
            ...
            else
            {
                ...
                do { if (0) rt_kprintf ("one block: block[0x%08x], next_free 0x%08x, prev_free 0x%08x\n", header_ptr, header_ptr->next_free, header_ptr->prev_free); } while (0)
            ...
            header_ptr->magic |= 0x01;


            rt_sem_release(&(heap->lock));


            do { if (0) rt_kprintf ("alloc mem: memory[0x%08x], heap[0x%08x], size: %d\n", (void *)((rt_uint8_t *)header_ptr + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1))), header_ptr, size); } while (0)



                                ;

            return (void *)((rt_uint8_t *)header_ptr + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
        }
    ...
    do { if (0) rt_kprintf ("allocate memory: failed\n"); } while (0);
...
void __attribute__((section(".fast"))) *rt_memheap_realloc(struct rt_memheap *heap, void *ptr, rt_size_t newsize)
{
    rt_err_t result;
    rt_size_t oldsize;
    struct rt_memheap_item *header_ptr;
    ...
    header_ptr = (struct rt_memheap_item *)
                 ((rt_uint8_t *)ptr - (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
    oldsize = ((rt_uint32_t)header_ptr->next - (rt_uint32_t)header_ptr - (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
    ...
    if (newsize > oldsize)
    {
        ...
        if (!(next_ptr > header_ptr)) { rt_assert_handler("next_ptr > header_ptr", __FUNCTION__, 335); };


        if (!((next_ptr)->magic & 0x01))
        {
            rt_int32_t nextsize;

            nextsize = ((rt_uint32_t)next_ptr->next - (rt_uint32_t)next_ptr - (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
            if (!(next_ptr > 0)) { rt_assert_handler("next_ptr > 0", __FUNCTION__, 343); };
# 354 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/src/memheap.c"
            if (nextsize + oldsize > newsize + 12)
            {
                ...
                do { if (0) rt_kprintf ("remove block: block[0x%08x], next_free 0x%08x, prev_free 0x%08x", next_ptr, next_ptr->next_free, next_ptr->prev_free); } while (0)
                ...
                do { if (0) rt_kprintf ("new free block: block[0x%08x] nextm[0x%08x] prevm[0x%08x]", next_ptr, next_ptr->next, next_ptr->prev); } while (0)
                ...
                do { if (0) rt_kprintf ("new ptr: next_free 0x%08x, prev_free 0x%08x", next_ptr->next_free, next_ptr->prev_free); } while (0)
    ...
    if (newsize + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)) + 12 >= oldsize)
        return ptr;
    ...
    new_ptr = (struct rt_memheap_item *)
              (((rt_uint8_t *)header_ptr) + newsize + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
    ...
    if (!((new_ptr->next)->magic & 0x01))
    {
        struct rt_memheap_item *free_ptr;


        free_ptr = new_ptr->next;
        heap->available_size = heap->available_size - ((rt_uint32_t)free_ptr->next - (rt_uint32_t)free_ptr - (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));

        do { if (0) rt_kprintf ("merge: right node 0x%08x, next_free 0x%08x, prev_free 0x%08x\n", header_ptr, header_ptr->next_free, header_ptr->prev_free); } while (0)
    ...
    heap->free_list->next_free->prev_free = new_ptr;
    heap->free_list->next_free = new_ptr;
    do { if (0) rt_kprintf ("new free ptr: next_free 0x%08x, prev_free 0x%08x\n", new_ptr->next_free, new_ptr->prev_free); } while (0)

                                                        ;


    heap->available_size = heap->available_size + ((rt_uint32_t)new_ptr->next - (rt_uint32_t)new_ptr - (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));
...
void __attribute__((section(".fast"))) rt_memheap_free(void *ptr)
{
    rt_err_t result;
    ...
    do { if (0) rt_kprintf ("free memory: memory[0x%08x], block[0x%08x]\n", ptr, header_ptr); } while (0)
                                                     ;


    if (!((header_ptr->magic & 0xfffffffe) == 0x1ea01ea0)) { rt_assert_handler("(header_ptr->magic & RT_MEMHEAP_MASK) == RT_MEMHEAP_MAGIC", __FUNCTION__, 519); };
    if (!(header_ptr->magic & 0x01)) { rt_assert_handler("header_ptr->magic & RT_MEMHEAP_USED", __FUNCTION__, 520); };

    if (!((header_ptr->next->magic & 0xfffffffe) == 0x1ea01ea0)) { rt_assert_handler("(header_ptr->next->magic & RT_MEMHEAP_MASK) == RT_MEMHEAP_MAGIC", __FUNCTION__, 522); };
    ...
    heap->available_size = heap->available_size + ((rt_uint32_t)header_ptr->next - (rt_uint32_t)header_ptr - (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1)));


    if (!((header_ptr->prev)->magic & 0x01))
    {
        do { if (0) rt_kprintf ("merge: left node 0x%08x\n", header_ptr->prev); } while (0)
                                                          ;


        heap->available_size = heap->available_size + (((sizeof(struct rt_memheap_item)) + (8) - 1) & ~((8) - 1));
    ...
    if (!((header_ptr->next)->magic & 0x01))
    {
        ...
        do { if (0) rt_kprintf ("merge: right node 0x%08x, next_free 0x%08x, prev_free 0x%08x\n", new_ptr, new_ptr->next_free, new_ptr->prev_free); } while (0)
    ...
    if (insert_header)
    {
        ...
        do { if (0) rt_kprintf ("insert to free list: next_free 0x%08x, prev_free 0x%08x\n", header_ptr->next_free, header_ptr->prev_free); } while (0)
