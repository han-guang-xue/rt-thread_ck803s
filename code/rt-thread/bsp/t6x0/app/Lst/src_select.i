# 1 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c"
...
# 1 "<command-line>"
# 1 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c"
# 24 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c"
...
# 25 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c" 2
...
# 26 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c" 2
...
# 27 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c" 2
...
# 29 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c" 2
...
# 30 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c" 2

int __attribute__((section(".fast"))) select(int nfds, _types_fd_set *readfds, _types_fd_set *writefds, _types_fd_set *exceptfds, struct timeval *timeout)
{
    int fd;
    int npfds;
    int msec;
    ...
    for (fd = 0, npfds = 0; fd < nfds; fd++)
    {
        ...
        if ((readfds && ((readfds)->fds_bits[(fd)/(sizeof (fd_mask) * 8)] & (1L << ((fd) % (sizeof (fd_mask) * 8))))) ||
            (writefds && ((writefds)->fds_bits[(fd)/(sizeof (fd_mask) * 8)] & (1L << ((fd) % (sizeof (fd_mask) * 8))))) ||
            (exceptfds && ((exceptfds)->fds_bits[(fd)/(sizeof (fd_mask) * 8)] & (1L << ((fd) % (sizeof (fd_mask) * 8))))))
        {
            npfds++;
        }
    }
    ...
    {
        int incr = 0;
# 74 "E:/YCXGIT/T6x0_RTOS_SDK_V0.9.3_b1/T6x0_RTOS_SDK_V0.9.3/code/rt-thread/components/dfs/src/select.c"
        if (readfds && ((readfds)->fds_bits[(fd)/(sizeof (fd_mask) * 8)] & (1L << ((fd) % (sizeof (fd_mask) * 8)))))
        {
            pollset[ndx].fd = fd;
            pollset[ndx].events |= (0x01);
            incr = 1;
        }

        if (writefds && ((writefds)->fds_bits[(fd)/(sizeof (fd_mask) * 8)] & (1L << ((fd) % (sizeof (fd_mask) * 8)))))
        {
            pollset[ndx].fd = fd;
            pollset[ndx].events |= (0x02);
            incr = 1;
        }

        if (exceptfds && ((exceptfds)->fds_bits[(fd)/(sizeof (fd_mask) * 8)] & (1L << ((fd) % (sizeof (fd_mask) * 8)))))
        {
            pollset[ndx].fd = fd;
            incr = 1;
        }
    ...
    if (!(ndx == npfds)) { rt_assert_handler("ndx == npfds", __FUNCTION__, 97); };
    ...
    if (readfds)
    {
        memset(readfds, 0, sizeof(_types_fd_set));
    }

    if (writefds)
    {
        memset(writefds, 0, sizeof(_types_fd_set));
    }

    if (exceptfds)
    {
        memset(exceptfds, 0, sizeof(_types_fd_set));
    }
    ...
    if (ret > 0)
    {
        ...
        for (ndx = 0; ndx < npfds; ndx++)
        {
            ...
            if (readfds)
            {
                if (pollset[ndx].revents & ((0x01) | (0x08)))
                {
                    ((readfds)->fds_bits[(pollset[ndx].fd)/(sizeof (fd_mask) * 8)] |= (1L << ((pollset[ndx].fd) % (sizeof (fd_mask) * 8))));
                    ret++;
                }
            }
            ...
            if (writefds)
            {
                if (pollset[ndx].revents & (0x02))
                {
                    ((writefds)->fds_bits[(pollset[ndx].fd)/(sizeof (fd_mask) * 8)] |= (1L << ((pollset[ndx].fd) % (sizeof (fd_mask) * 8))));
                    ret++;
                }
            }
            ...
            if (exceptfds)
            {
                if (pollset[ndx].revents & (0x04))
                {
                    ((exceptfds)->fds_bits[(pollset[ndx].fd)/(sizeof (fd_mask) * 8)] |= (1L << ((pollset[ndx].fd) % (sizeof (fd_mask) * 8))));
                    ret++;
                }
            }
        }
