...
# 26 "../../../packages/webnet-2.0.0/src/wn_session.c" 2
...
# 27 "../../../packages/webnet-2.0.0/src/wn_session.c" 2
...
# 28 "../../../packages/webnet-2.0.0/src/wn_session.c" 2
...
# 30 "../../../packages/webnet-2.0.0/src/wn_session.c" 2
...
# 31 "../../../packages/webnet-2.0.0/src/wn_session.c" 2
...
# 32 "../../../packages/webnet-2.0.0/src/wn_session.c" 2

static struct webnet_session *_session_list = 0;

static void (*webnet_err_callback)(struct webnet_session *session);
# 44 "../../../packages/webnet-2.0.0/src/wn_session.c"
struct webnet_session* webnet_session_create(int listenfd)
{
    struct webnet_session* session;


    session = (struct webnet_session *)rt_malloc(sizeof(struct webnet_session));
    if (session != (0))
    {
        ...
        extern tls_srv_context *wn_tls_context;
        session->tls_session = tls_srv_session_create(wn_tls_context);
        ...
        tls_srv_session_init(wn_tls_context, session->tls_session);

        session->socket = session->tls_session->cli_fd.fd;

        if (tls_srv_session_config(session->tls_session) != 0)
        {
            tls_srv_session_close(session->tls_session);
            tls_srv_session_destroy(session->tls_session);
            rt_free(session);
            session = (0);
        ...
        session->buffer_length = (4 * 1024);
    }

    return session;
}
# 113 "../../../packages/webnet-2.0.0/src/wn_session.c"
int webnet_session_read(struct webnet_session *session, char *buffer, int length)
{
    int read_count;


    read_count = tls_srv_session_read(session->tls_session, (unsigned char *)buffer, length);
...
void webnet_session_close(struct webnet_session *session)
{
    ...
    if (session->session_ops != (0) &&
            session->session_ops->session_close != (0))
    {
    ...
    if (tls_srv_session_close(session->tls_session) != 0)
        return;
    tls_srv_session_destroy(session->tls_session);
...
void webnet_session_printf(struct webnet_session *session, const char* fmt, ...)
{
    ...
    tls_srv_session_write(session->tls_session, session->buffer, length);



}
;
# 219 "../../../packages/webnet-2.0.0/src/wn_session.c"
int webnet_session_write(struct webnet_session *session, const rt_uint8_t* data, rt_size_t size)
{

    tls_srv_session_write(session->tls_session, data, size);
...
;
# 240 "../../../packages/webnet-2.0.0/src/wn_session.c"
int webnet_session_redirect(struct webnet_session *session, const char* url)
{
    struct webnet_request *request;

    if (!(session != (0))) { rt_assert_handler("session != RT_NULL", __FUNCTION__, 244); };
    request = session->request;
    if (!(request != (0))) { rt_assert_handler("request != RT_NULL", __FUNCTION__, 246); };


    if (request->path != (0))
        rt_free(request->path);
    request->path = rt_strdup(url);


    return webnet_module_handle_uri(session);
}
;
# 268 "../../../packages/webnet-2.0.0/src/wn_session.c"
int webnet_session_get_physical_path(struct webnet_session *session, const char* virtual_path, char* full_path)
{
    int result;
    ...
    rt_snprintf(full_path, 256, "%s/%s", webnet_get_root(), virtual_path);
...
;
# 298 "../../../packages/webnet-2.0.0/src/wn_session.c"
        ...
        const char * reason_phrase)
{
    ...
    webnet_session_printf(session, "Server: webnet ""2.0.0""\r\n");
}
;
# 321 "../../../packages/webnet-2.0.0/src/wn_session.c"
void webnet_session_set_header(struct webnet_session *session, const char* mimetype, int code, const char* title, int length)
{
    ...
    offset = rt_snprintf(ptr, end_buffer - ptr, fmt, code, title, "Server: webnet ""2.0.0""\r\n");
    ptr += offset;
    ...
    if (webnet_module_handle_event(session, (1 << 3)) == 0)
    {
...
static void _webnet_session_handle(struct webnet_session* session, int event)
{
    ...
    case (1 << 6):
        ...
        if (session->session_phase == WEB_PHASE_RESPONSE)
        {
            ...
            session->session_ops = 
# 448 "../../../packages/webnet-2.0.0/src/wn_session.c" 3 4
                                  (0)
# 448 "../../../packages/webnet-2.0.0/src/wn_session.c"
                                      ;
            session->user_data = 0;
...
# 523 "../../../packages/webnet-2.0.0/src/wn_session.c"
int webnet_sessions_set_fds(_types_fd_set *readset, _types_fd_set *writeset)
{
    int maxfdp1 = 0;
    struct webnet_session *session;
    ...
    for (session = _session_list; session; session = session->next)
    {
        ...
        ((readset)->fds_bits[(session->socket)/(sizeof (fd_mask) * 8)] |= (1L << ((session->socket) % (sizeof (fd_mask) * 8))));
        if (session->session_event_mask & (1 << 7))
            ((writeset)->fds_bits[(session->socket)/(sizeof (fd_mask) * 8)] |= (1L << ((session->socket) % (sizeof (fd_mask) * 8))));
    }
...
void webnet_sessions_handle_fds(_types_fd_set *readset, _types_fd_set *writeset)
{
    ...
    for (session = _session_list; session; session = next_session)
    {
        ...
        if (((readset)->fds_bits[(session->socket)/(sizeof (fd_mask) * 8)] & (1L << ((session->socket) % (sizeof (fd_mask) * 8)))))
        {
            if (session->session_ops == (0))
            {
                struct webnet_request *request;
            ...
            if (session->session_ops == (0) || session->session_phase == WEB_PHASE_CLOSE)
            {

                if (session->request->result_code != 200)
                {

                    if (webnet_err_callback != (0))
                    {
        ...
        else if (((writeset)->fds_bits[(session->socket)/(sizeof (fd_mask) * 8)] & (1L << ((session->socket) % (sizeof (fd_mask) * 8)))))
        {

            if (session->session_ops != (0) &&
                session->session_ops->session_handle != (0))
            {
                session->session_ops->session_handle(session, (1 << 7));
            }
...
# 644 "../../../packages/webnet-2.0.0/src/wn_session.c" 2
...
static void list_webnet(void)
{
    ...
    rt_enter_critical();
    for (session = _session_list; session != (0); session = session->next)
    {
        strcpy(client_ip_str,
               ip4addr_ntoa((const ip4_addr_t*)&(*((struct in_addr*)&(session->cliaddr.sin_addr)))));

        rt_kprintf("#%u client %s:%u \n",
                   num++,
                   client_ip_str,
                   lwip_htons(session->cliaddr.sin_port));
...
const char __fsym___cmd_list_webnet_name[] __attribute__((section(".rodata.name"))) = "__cmd_list_webnet"; const char __fsym___cmd_list_webnet_desc[] __attribute__((section(".rodata.name"))) = "list webnet session"; const struct finsh_syscall __fsym___cmd_list_webnet __attribute__((section("FSymTab")))= { __fsym___cmd_list_webnet_name, __fsym___cmd_list_webnet_desc, (syscall_func)&list_webnet };;
