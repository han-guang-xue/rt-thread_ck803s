# 1 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
# 1 "E:\\YCXGIT\\T6x0_RTOS_SDK_V0.9.3_b1\\T6x0_RTOS_SDK_V0.9.3\\code\\rt-thread\\bsp\\t6x0\\app//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
# 42 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
...
# 43 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c" 2
...
# 34 "../../../packages/mbedtls-2.6.0/mbedtls/include/mbedtls/rsa.h" 2
...
# 48 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c" 2
...
# 49 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c" 2
...
# 51 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c" 2


# 1 "../../../packages/mbedtls-2.6.0/mbedtls/include/mbedtls/md.h" 1
# 54 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c" 2
# 63 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
...
# 64 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c" 2
...
# 70 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
                 ...
                 unsigned int nbits, int exponent )
{
    ...
    unsigned char q[buflen];
    unsigned char dp[buflen];
    unsigned char dq[buflen];
    unsigned char qp[buflen];
    unsigned char n[buflen];
    unsigned char e[buflen];

    ret = tih_rsa_keypair_gen(p,q,dp,dq,qp,e,n,nbits);
    if(0 != ret)
        return -0x4180;

    do { if( ( ret = mbedtls_mpi_read_binary( &ctx->P, p, buflen ) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_read_binary( &ctx->Q, q, buflen ) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_read_binary( &ctx->DP, dp, buflen ) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_read_binary( &ctx->DQ, dq, buflen ) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_read_binary( &ctx->QP, qp, buflen ) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_read_binary( &ctx->E, e, buflen ) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_read_binary( &ctx->N, n, buflen ) ) != 0 ) goto cleanup; } while( 0 );
...
int mbedtls_rsa_public( mbedtls_rsa_context *ctx,
                ...
                unsigned char *output )
{
    unsigned char N[ctx->N.n*sizeof(mbedtls_mpi_uint)];
    unsigned char E[ctx->E.n*sizeof(mbedtls_mpi_uint)];

    unsigned char out[ctx->N.n*sizeof(mbedtls_mpi_uint)];

    memset( N, 0, ctx->N.n*sizeof(mbedtls_mpi_uint) );
    memset( E, 0, ctx->E.n*sizeof(mbedtls_mpi_uint) );



    mbedtls_mpi_write_binary( &ctx->E, E, ctx->E.n*sizeof(mbedtls_mpi_uint) );
    mbedtls_mpi_write_binary( &ctx->N, N, ctx->N.n*sizeof(mbedtls_mpi_uint) );

    int ret = 0;
    ret = tih_rsa_encrypt( (unsigned char *)input, E, N, out, mbedtls_mpi_bitlen( &ctx->N ) );
    memcpy(output,out,ctx->N.n*sizeof(mbedtls_mpi_uint));
    return ret;
}
...
# 344 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
int mbedtls_rsa_private( mbedtls_rsa_context *ctx,
                 int (*f_rng)(void *, unsigned char *, size_t),
                 void *p_rng,
                 const unsigned char *input,
                 unsigned char *output )
{
    ...
    unsigned char P[ctx->P.n*sizeof(uint32_t)];
    unsigned char Q[ctx->Q.n*sizeof(uint32_t)];

    unsigned char DP[ctx->DP.n*sizeof(uint32_t)];
    unsigned char DQ[ctx->DQ.n*sizeof(uint32_t)];

    unsigned char QP[ctx->QP.n*sizeof(uint32_t)];



    memset( P, 0, ctx->P.n*sizeof(uint32_t) );
    memset( Q, 0, ctx->Q.n*sizeof(uint32_t) );
    memset( DP, 0, ctx->DP.n*sizeof(uint32_t) );
    memset( DQ, 0, ctx->DQ.n*sizeof(uint32_t) );
    memset( QP, 0, ctx->QP.n*sizeof(uint32_t) );

    mbedtls_mpi_write_binary( &ctx->P, P, ctx->P.n*sizeof(mbedtls_mpi_uint) );
    mbedtls_mpi_write_binary( &ctx->Q, Q, ctx->Q.n*sizeof(mbedtls_mpi_uint) );
    mbedtls_mpi_write_binary( &ctx->DP, DP, ctx->DP.n*sizeof(mbedtls_mpi_uint) );
    mbedtls_mpi_write_binary( &ctx->DQ, DQ, ctx->DQ.n*sizeof(mbedtls_mpi_uint) );
    mbedtls_mpi_write_binary( &ctx->QP, QP, ctx->QP.n*sizeof(mbedtls_mpi_uint) );

    int ret = tih_rsa_decrypt( (unsigned char *)input, P, Q, DP, DQ, QP, out, mbedtls_mpi_bitlen( &ctx->N ) );
...
# 392 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
                            ...
                            unsigned char *output )
{
    ...
    *p++ = 1;
    ...
    if( ( ret = mbedtls_md_setup( &md_ctx, md_info, ((1 << 16)) ) ) != 0 )
    {
...
# 975 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
int mbedtls_rsa_rsassa_pkcs1_v15_sign( mbedtls_rsa_context *ctx,
                               int (*f_rng)(void *, unsigned char *, size_t),
                               void *p_rng,
                               int mode,
                               ...
                               unsigned char *sig )
{
    ...
    *p++ = 0;
    ...
    {
# 1039 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
        *p++ = 0x10 | 0x20;
...
# 1496 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c" 2
# 1552 "../../../packages/mbedtls-2.6.0/mbedtls/library/rsa.c"
...
int mbedtls_rsa_self_test( int verbose )
{
    ...
    if( mbedtls_rsa_pkcs1_encrypt( &rsa, myrand, ((void *)0), 0, 24,
    ...
    if( mbedtls_rsa_pkcs1_decrypt( &rsa, myrand, ((void *)0), 1, &len,
    ...
    if( mbedtls_rsa_pkcs1_sign( &rsa, myrand, ((void *)0), 1, MBEDTLS_MD_SHA1, 0,
    ...
    if( mbedtls_rsa_pkcs1_verify( &rsa, ((void *)0), ((void *)0), 0, MBEDTLS_MD_SHA1, 0,
                          sha1sum, rsa_ciphertext ) != 0 )
    {
        if( verbose != 0 )
            printf( "failed\n" );
