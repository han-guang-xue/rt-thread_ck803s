# 1 "../../../packages/webnet-2.0.0/module/wn_module_alias.c"
# 1 "E:\\YCXGIT\\T6x0_RTOS_SDK_V0.9.3_b1\\T6x0_RTOS_SDK_V0.9.3\\code\\rt-thread\\bsp\\t6x0\\app//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "../../../packages/webnet-2.0.0/module/wn_module_alias.c"
# 24 "../../../packages/webnet-2.0.0/module/wn_module_alias.c"
...
# 25 "../../../packages/webnet-2.0.0/module/wn_module_alias.c" 2
...
# 26 "../../../packages/webnet-2.0.0/module/wn_module_alias.c" 2
...
# 27 "../../../packages/webnet-2.0.0/module/wn_module_alias.c" 2
...
static struct webnet_alias_item *_alias_items = (0);
...
void webnet_alias_set(char* old_path, char* new_path)
{
    ...
    {
        _alias_item_count = 1;
        _alias_items = (struct webnet_alias_item*)rt_malloc (sizeof(struct webnet_alias_item) *
                       _alias_item_count);
    }
    else
    {
        _alias_item_count += 1;
        _alias_items = (struct webnet_alias_item*) rt_realloc (_alias_items, sizeof(struct webnet_alias_item) *
                       _alias_item_count);
    }

    if (!(_alias_items != (0))) { rt_assert_handler("_alias_items != RT_NULL", __FUNCTION__, 53); };

    _alias_items[_alias_item_count - 1].old_path = rt_strdup(old_path);
    if (!(_alias_items[_alias_item_count - 1].old_path != (0))) { rt_assert_handler("_alias_items[_alias_item_count - 1].old_path != RT_NULL", __FUNCTION__, 56); };
    _alias_items[_alias_item_count - 1].new_path = rt_strdup(new_path);
    if (!(_alias_items[_alias_item_count - 1].new_path != (0))) { rt_assert_handler("_alias_items[_alias_item_count - 1].new_path != RT_NULL", __FUNCTION__, 58); };
}
;
...
int webnet_module_alias(struct webnet_session* session, int event)
{
    if (event == (1 << 1))
    {
        ...
        if (!(session != (0))) { rt_assert_handler("session != RT_NULL", __FUNCTION__, 69); };
        request = session->request;
        if (!(request != (0))) { rt_assert_handler("request != RT_NULL", __FUNCTION__, 71); };
        ...
        for (index = 0; index < _alias_item_count; index ++)
        {
            ...
            {
                char* map_path;
                map_path = (char*) rt_malloc (256);
                if (!(map_path != (0))) { rt_assert_handler("map_path != RT_NULL", __FUNCTION__, 80); };

                rt_snprintf(map_path, 256, "%s/%s",
