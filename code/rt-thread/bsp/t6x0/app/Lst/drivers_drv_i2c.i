...
# 4 "../drivers/drv_i2c.c" 2
# 1 "../libraries/inc/tih/i2c.h" 1
# 23 "../libraries/inc/tih/i2c.h"
typedef struct i2c_config {
    unsigned int speed;
} i2c_config_t;

void i2c_intr_enable(unsigned int intr_src);
void i2c_intr_disable(unsigned int intr_src);
unsigned int i2c_intr_status(void);
void i2c_intr_clear(unsigned int intr_src);
unsigned int i2c_status_get(void);
int i2c_hw_init(i2c_config_t *config);
int i2c_is_busy(void);
int i2c_send(unsigned char data, unsigned char start, unsigned char stop);
int i2c_recv(unsigned char *data, unsigned int stop);
void i2c_scl_val_set(unsigned int val);
void i2c_sda_val_set(unsigned int val);
# 5 "../drivers/drv_i2c.c" 2
...
static rt_size_t __attribute__((section(".fast"))) drv_i2c_xfer(struct rt_i2c_bus_device *bus,
                              struct rt_i2c_msg msgs[], rt_uint32_t num)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        if (msg->flags & (1u << 0)) {
            ...
            msg->addr = (msg->addr << 1) | 0x01;
            ret = i2c_send((char)msg->addr, 1, 0);
            if (ret) {
                return 0;
            }
...
const init_fn_t __rt_init_drv_i2c_init __attribute__((section(".rti_fn.""3"))) = drv_i2c_init;
