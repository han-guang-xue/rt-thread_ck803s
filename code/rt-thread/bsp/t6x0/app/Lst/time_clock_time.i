...
# 27 "../../../components/libc/time/clock_time.c" 2
...
# 28 "../../../components/libc/time/clock_time.c" 2
# 1 "../../../components/libc/time/clock_time.h" 1
# 54 "../../../components/libc/time/clock_time.h"
int clock_getres (clockid_t clockid, struct timespec *res);
int clock_gettime (clockid_t clockid, struct timespec *tp);
int clock_settime (clockid_t clockid, const struct timespec *tp);
# 29 "../../../components/libc/time/clock_time.c" 2
...
int clock_time_system_init()
{
    ...
    _timevalue.tv_usec = (tick%100) * (1000000UL / 100);
    _timevalue.tv_sec = time - tick/100 - 1;

    return 0;
}
const init_fn_t __rt_init_clock_time_system_init __attribute__((section(".rti_fn.""4"))) = clock_time_system_init;

int __attribute__((section(".fast"))) clock_time_to_tick(const struct timespec *time)
{
    int tick;
    int nsecond, second;
    struct timespec tp;

    if (!(time != (0))) { rt_assert_handler("time != RT_NULL", __FUNCTION__, 61); };
    ...
    if (time != ((void *)0))
    {
        ...
        {
            nsecond = 1000000000UL - (tp.tv_nsec - time->tv_nsec);
        ...
        tick = second * 100 + nsecond * 100 / 1000000000UL;
        if (tick < 0) tick = 0;
    }
...
int __attribute__((section(".fast"))) clock_getres(clockid_t clockid, struct timespec *res)
{
    int ret = 0;
...
int __attribute__((section(".fast"))) clock_gettime(clockid_t clockid, struct timespec *tp)
{
    int ret = 0;
    ...
    case 1:
        {
            ...
            tp->tv_sec = _timevalue.tv_sec + tick / 100;
            tp->tv_nsec = (_timevalue.tv_usec + (tick % 100) * (1000000UL / 100)) * 1000;
        }
        break;
    ...
    case 2:
        {
            ...
            tp->tv_sec = ((int)(cpu_tick * unit)) / 1000000000UL;
            tp->tv_nsec = ((int)(cpu_tick * unit)) % 1000000000UL;
        }
        break;
...
int __attribute__((section(".fast"))) clock_settime(clockid_t clockid, const struct timespec *tp)
{
    int second;
    rt_tick_t tick;
    rt_device_t device;
    ...
    _timevalue.tv_sec = second - tick/100 - 1;
    ...
    if (device != (0))
    {
        ...
        rt_device_control(device, 0x11, &second);
    }
    else
        return -1;
