# 1 "../../../components/drivers/spi/spi_adc.c"
...
# 2 "../../../components/drivers/spi/spi_adc.c" 2
...
# 3 "../../../components/drivers/spi/spi_adc.c" 2
...
# 4 "../../../components/drivers/spi/spi_adc.c" 2
# 1 "../libraries/inc/tih/scu.h" 1



# 1 "../libraries/inc/tih/sysdep.h" 1
# 5 "../libraries/inc/tih/scu.h" 2
...
# 5 "../../../components/drivers/spi/spi_adc.c" 2
...
{
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 36); };
...
static rt_size_t mcp3202_adc_read(rt_device_t dev,
                                   ...
                                   rt_size_t size)
{
    unsigned char send_buff[3], recv_buff[3];
    ...
    send_buff[0] = 0x01;
    send_buff[1] = mcp3202_input_mode;
    send_buff[2] = 0x00;
    ...
    while ((size > 0) && (size > 1))
    {
        rt_mutex_take(&spi_adc_device.lock, 0);
        rt_spi_transfer_message(spi_adc_device.rt_spi_device, &msg);
        rt_mutex_release(&spi_adc_device.lock);

        ad_value = ((recv_buff[1] << 8) | recv_buff[2]) & 0x0fff;
        *(unsigned short *)buffer = ad_value;
        buffer = buffer + 2;
        size = size -2;
        count++;
        rt_thread_mdelay(10);
...
# 107 "../../../components/drivers/spi/spi_adc.c"
int mcp3202_init(void)
{
   static struct rt_spi_device *rt_spi_device;
    ...
    if (rt_mutex_init(&spi_adc_device.lock, spi_device_name, 0x00) != 0)
    {
    ...
    rt_spi_bus_attach_device(rt_spi_device, spi_device_name, "spi1", (void *)((void *)0));
    ...
    spi_adc_device.adc_device.user_data = (0);
...
const init_fn_t __rt_init_mcp3202_init __attribute__((section(".rti_fn.""3"))) = mcp3202_init;
