# 1 "../../../components/drivers/sata/sata.c"
...
void xmem_init(void);
# 2 "../../../components/drivers/sata/sata.c" 2
...
# 4 "../../../components/drivers/sata/sata.c" 2




# 1 "../../../include/rtdbg.h" 1
# 9 "../../../components/drivers/sata/sata.c" 2
...
static rt_err_t rt_ata_init(rt_device_t dev)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 16); };
    ...
    rt_mutex_init(&ata->dlock, "ata_lock", 0x00);
...
static rt_err_t __attribute__((section(".fast"))) rt_ata_open(rt_device_t dev, rt_uint16_t oflag)
{
    return 0;
}

static rt_err_t __attribute__((section(".fast"))) rt_ata_close(rt_device_t dev)
{
    return 0;
}

static rt_size_t __attribute__((section(".fast"))) rt_ata_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
    rt_size_t nsector;
    rt_ata_device_t *ata;

    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 59); };

    ata = (rt_ata_device_t *)dev;

    rt_mutex_take(&ata->dlock, -1);
...
static rt_size_t __attribute__((section(".fast"))) rt_ata_write(rt_device_t dev, rt_off_t pos, const void *buffer, rt_size_t size)
{
    rt_size_t nsector;
    rt_ata_device_t *ata;

    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 81); };
...
static rt_err_t __attribute__((section(".fast"))) rt_ata_ioctl(rt_device_t dev, int cmd, void *args)
{
    rt_err_t result = 0;
    rt_ata_device_t *ata;
    ...
    rt_uint32_t *ptbl;
    rt_size_t size;
    static __attribute__((section(".exdata"),zero_init)) rt_uint32_t table[2];

    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 107); };
...
# 160 "../../../components/drivers/sata/sata.c"
rt_err_t rt_ata_register(struct rt_ata_device *ata,
                               const char *name,
                               rt_uint32_t flag,
                               void *data)
{
    ...
    if (!(ata != (0))) { rt_assert_handler("ata != RT_NULL", __FUNCTION__, 168); };
