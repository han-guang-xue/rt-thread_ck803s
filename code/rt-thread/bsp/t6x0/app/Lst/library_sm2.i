# 1 "../../../packages/mbedtls-2.6.0/mbedtls/library/sm2.c"
...
# 5 "../../../packages/mbedtls-2.6.0/mbedtls/library/sm2.c" 2
...
# 13 "../../../packages/mbedtls-2.6.0/mbedtls/library/sm2.c" 2
...
# 17 "../../../packages/mbedtls-2.6.0/mbedtls/library/sm2.c" 2
...
# 18 "../../../packages/mbedtls-2.6.0/mbedtls/library/sm2.c" 2
...
int mbedtls_sm2_genkey( mbedtls_ecp_group *grp, mbedtls_mpi *d, mbedtls_ecp_point *Q )
{
    ...
    if(ret == 0){
        do { if( ( ret = mbedtls_mpi_read_binary( d, prikey, nbytes ) ) != 0 ) goto cleanup; } while( 0 );
        do { if( ( ret = mbedtls_mpi_read_binary( &Q->X, pubkey, nbytes ) ) != 0 ) goto cleanup; } while( 0 );
        do { if( ( ret = mbedtls_mpi_read_binary( &Q->Y, pubkey + nbytes, nbytes ) ) != 0 ) goto cleanup; } while( 0 );
        do { if( ( ret = mbedtls_mpi_lset( &Q->Z, 1 ) ) != 0 ) goto cleanup; } while( 0 );
    }
    return ret;
                         ...
                         const unsigned char *z, size_t zlen, unsigned char *e, size_t elen)
{
    ...
    ret = tih_sm2_e_get( (unsigned char *)m, mlen, (unsigned char *)z, e );
                         ...
                         unsigned char *z, size_t zlen)
{
    ...
    nbytes = ( ( grp->pbits + 7 ) / 8 );
    uint8_t pubkey[nbytes << 1];

    do { if( ( ret = mbedtls_mpi_write_binary( &Q->X, pubkey, nbytes) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_write_binary( &Q->Y, pubkey + nbytes, nbytes) ) != 0 ) goto cleanup; } while( 0 );

    ret = tih_sm2_z_get( (unsigned char *)id, ilen, pubkey, z );
                         ...
                         unsigned char *buf, size_t *blen)
{
    ...
    ret = tih_sm2_encrypt( (unsigned char *)m, mlen, pubkey, buf, blen);
                         ...
                         size_t blen, unsigned char *m, size_t *mlen)
{
    ...
    do { if( ( ret = mbedtls_mpi_write_binary( d, prikey, nbytes) ) != 0 ) goto cleanup; } while( 0 );
    ret = tih_sm2_decrypt( (unsigned char *)buf, blen, prikey, m, mlen);
...
int mbedtls_sm2_self_test( int verbose ){
    ...
    printf("SM2 encrypt test:");
    do { if( ( ret = mbedtls_mpi_read_binary( &Q.X, &KEY_PUB[0], 32 ) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_mpi_read_binary( &Q.Y, &KEY_PUB[32], 32 ) ) != 0 ) goto cleanup; } while( 0 );

    do { if( ( ret = mbedtls_sm2_encrypt( &grp, &Q, PLAIN, sizeof(PLAIN), buf, &blen ) ) != 0 ) goto cleanup; } while( 0 );

    printf("passed\nSM2 decrypt test:");

    do { if( ( ret = mbedtls_mpi_read_binary( &d, KEY_PRI, 32) ) != 0 ) goto cleanup; } while( 0 );
    do { if( ( ret = mbedtls_sm2_decrypt( &grp, &d, CIPHER, sizeof(CIPHER), encrypt, &blen ) ) != 0 ) goto cleanup; } while( 0 );
