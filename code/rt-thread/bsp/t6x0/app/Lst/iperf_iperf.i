# 1 "../../../packages/netutils-1.0.0/iperf/iperf.c"
...
# 7 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
...
# 10 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
...
# 12 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
...
# 15 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
# 1 "d:\\c-sky\\cdk\\csky\\mingw\\csky-abiv2-elf-toolchain\\csky-elfabiv2\\sys-include\\fcntl.h" 1 3 4
# 55 "d:\\c-sky\\cdk\\csky\\mingw\\csky-abiv2-elf-toolchain\\csky-elfabiv2\\sys-include\\fcntl.h" 3 4
extern int open(const char *pathname, int flags, ...);

extern int fcntl (int __fd, int __cmd, ...);
# 16 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
...
# 20 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
...
# 21 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
...
int getaddrinfo(const char *nodename,
       ...
       const struct addrinfo *hints,
       struct addrinfo **res);
# 22 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
# 30 "../../../packages/netutils-1.0.0/iperf/iperf.c"
typedef struct
{
    int mode;
...
static IPERF_PARAM param = {0, ((void *)0), 5001};

static void __attribute__((section(".fast"))) iperf_udp_client(void *thread_param)
{
    int sock;
    rt_uint32_t *buffer;
    ...
    server.sin_addr.s_addr = ipaddr_addr(param.host);
    ...
    while (param.mode != 0)
    {
        ...
        sendto(sock, buffer, 1470, 0, (struct sockaddr *)&server, sizeof(struct sockaddr_in));
    }
    closesocket(sock);
    free(buffer);
}

static void __attribute__((section(".fast"))) iperf_udp_server(void *thread_param)
{
    int sock;
    rt_uint32_t *buffer;
    struct sockaddr_in server;
    ...
    server.sin_family = 2;
    server.sin_port = lwip_htons(param.port);
    server.sin_addr.s_addr = ipaddr_addr("0.0.0.0");

    timeout.tv_sec = 2;
    timeout.tv_usec = 0;
    if (setsockopt(sock, 0xfff, 0x1006, &timeout, sizeof(timeout)) == -1)
    {
    ...
    while (param.mode != 0)
    {
        ...
        total = 0;
        sentlen = 0;
        while ((tick2 - tick1) < (100 * 5))
        {
            r_size = recvfrom(sock, buffer, (4 * 1024), 0, (struct sockaddr *)&sender, (socklen_t*)&sender_len);
            if (r_size > 12)
            {
                pcount = lwip_htonl(buffer[0]);
        ...
        if (sentlen > 0)
        {
            f = (float)(sentlen * 100 / 125 / (tick2 - tick1));
            f /= 1000.0f;
...
static void __attribute__((section(".fast"))) iperf_client(void *thread_param)
{
    int i;
    int sock;
    int ret;
    ...
    send_buf = (uint8_t *) malloc((4 * 1024));
    if (!send_buf) return ;
    ...
    while (param.mode != 0)
    {
        ...
        addr.sin_addr.s_addr = ipaddr_addr((char *)param.host);
        ...
        while (param.mode != 0)
        {
            ...
            ret = send(sock, send_buf, (4 * 1024), 0);
            if (ret > 0)
            {
                sentlen += ret;
            }
...
void __attribute__((section(".fast"))) iperf_server(void *thread_param)
{
    uint8_t *recv_data;
    socklen_t sin_size;
    rt_tick_t tick1, tick2;
    ...
    recv_data = (uint8_t *)malloc((4 * 1024));
    if (recv_data == (0))
    {
    ...
    server_addr.sin_port = lwip_htons(param.port);
    ...
    while (param.mode != 0)
    {
        ...
        if (select(sock + 1, &readset, (0), (0), &timeout) == 0)
            continue;
        ...
        while (param.mode != 0)
        {
            bytes_received = recv(connected, recv_data, (4 * 1024), 0);
            ...
            if (tick2 - tick1 >= 100 * 5)
            {
                ...
                f = (float)(recvlen * 100 / 125 / (tick2 - tick1));
                f /= 1000.0f;
                snprintf(speed, sizeof(speed), "%.4f Mbps!\n", f);
                rt_kprintf("%s", speed);
                tick1 = tick2;
...
int iperf(int argc, char **argv)
{
    ...
    if (param.mode == 0)
    {
        ...
        if (use_udp)
        {
            if (mode == 2)
            {
                tid = rt_thread_create("iperfc", iperf_udp_client, (0),
                                   2048, 20-5, 20);
            }
            else if (mode == 1)
            {
                tid = rt_thread_create("iperfd", iperf_udp_server, (0),
                                   2048, 10, 20);
            }
        }
        else
        {
            if (mode == 2)
            {
                tid = rt_thread_create("iperfc", iperf_client, (0),
                                   2048, 20-2, 20);
            }
            else if (mode == 1)
            {
                tid = rt_thread_create("iperfd", iperf_server, (0),
                                   2048, 10, 20);
            }
        }
...
# 504 "../../../packages/netutils-1.0.0/iperf/iperf.c" 2
    const char __fsym___cmd_iperf_name[] __attribute__((section(".rodata.name"))) = "__cmd_iperf"; const char __fsym___cmd_iperf_desc[] __attribute__((section(".rodata.name"))) = "the network bandwidth measurement tool"; const struct finsh_syscall __fsym___cmd_iperf __attribute__((section("FSymTab")))= { __fsym___cmd_iperf_name, __fsym___cmd_iperf_desc, (syscall_func)&iperf };;
