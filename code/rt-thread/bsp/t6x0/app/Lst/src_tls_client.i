# 1 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
# 1 "E:\\YCXGIT\\T6x0_RTOS_SDK_V0.9.3_b1\\T6x0_RTOS_SDK_V0.9.3\\code\\rt-thread\\bsp\\t6x0\\app//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
# 19 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
...
# 20 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 2
...
# 21 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 2
...
# 22 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 2
...
# 26 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 2
...
# 27 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 2
# 40 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
tls_cli_session* tls_cli_session_create(char *host, char *port, int buf_size)
{
    ...
    if (!host || !port || (buf_size <= 0)) {
        return 
# 45 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
              (0)
# 45 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                  ;
    }

    cli_session = (tls_cli_session*)malloc(sizeof(tls_cli_session));
    if (!cli_session) {
        rt_kprintf("[tls]");rt_kprintf("[fail]no memory for cli_session struct\n");
        return 
# 51 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
              (0)
# 51 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                  ;
    }
    ...
    if (!cli_session->buffer) {
        rt_kprintf("[tls]");rt_kprintf("[fail]no memory for tls_session->buffer malloc\n");
        tls_cli_session_destroy(cli_session);
        return 
# 62 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
              (0)
# 62 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                  ;
    }
...
int tls_cli_session_init(tls_cli_session *session, int suite_preset)
{
    ...
    mbedtls_debug_set_threshold((int)(10));
    ...
    rt_kprintf("[tls]");rt_kprintf("mbedtls client struct init success...\n");
    return 0;
}
...
int tls_cli_session_config(tls_cli_session *session)
{
    ...
    rt_kprintf("[tls]");rt_kprintf("Loading the CA root certificate success...\n");
    if ((session->suite_preset == 0) ||
     (session->suite_preset == 2)) {

        ret = mbedtls_x509_crt_parse(&session->crt, (const unsigned char*)mbedtls_test_srv_crt,
                                     mbedtls_test_srv_crt_len);
        if (ret != 0) {
            rt_kprintf("[tls]");rt_kprintf("[fail] mbedtls_x509_crt_parse returned %d\n", ret);
            return -1;
        }
...
# 151 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
                                                            (0)
# 151 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                                                                , 0);
        if (ret != 0) {
            rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_pk_parse_key returned %d\n", ret);
            return -1;
        }
        ...
        ret = mbedtls_gm_pk_parse_key(&session->gm_pkey_dsa,
                                      ...
                                      mbedtls_gmtest_cli_dsa_key_len, 
# 182 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
                                                                     (0)
# 182 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                                                                         , 0);
        if (ret != 0) {
            rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_pk_parse_key returned %d\n", ret);
            return -1;
        ...
        ret = mbedtls_gm_pk_parse_key(&session->gm_pkey_enc,
                                      ...
                                      mbedtls_gmtest_cli_enc_key_len, 
# 190 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
                                                                     (0)
# 190 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                                                                         , 0);
        if (ret != 0) {
            rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_pk_parse_key returned %d\n", ret);
            return -1;
    ...
    if ((ret = mbedtls_ssl_set_hostname(&session->ssl, session->host)) != 0) {
        rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_ssl_set_hostname err returned -0x%x\n", -ret);
        return ret;
    }

    if ((ret = mbedtls_ssl_config_defaults(&session->conf,
                                           0,
                                           0, session->suite_preset)) != 0) {
        rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_ssl_config_defaults returned -0x%x\n", -ret);
        return ret;
    }

    mbedtls_ssl_conf_authmode(&session->conf, 1);

    if ((session->suite_preset == 0) ||
     (session->suite_preset == 2)) {
        mbedtls_ssl_conf_ca_chain(&session->conf, session->crt.next, 
# 215 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
                                                                    (0)
# 215 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                                                                        );
        if ((ret = mbedtls_ssl_conf_own_cert(&session->conf, &session->crt, &session->pkey)) != 0) {
            rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_ssl_conf_own_cert returned %d\n", ret);
            return -1;
        }
    } else {

        mbedtls_gmssl_conf_ca_chain(&session->conf, session->gm_crt.next->next, 
# 222 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
                                                                               (0)
# 222 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                                                                                   );
...
# 238 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
                                                                 (0)
# 238 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                                                                     );

    if ((ret = mbedtls_ssl_setup(&session->ssl, &session->conf)) != 0) {
        rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_ssl_setup returned -0x%x\n", -ret);
        return ret;
    }

    rt_kprintf("[tls]");rt_kprintf("mbedtls client context init success...\n");
    return 0;
}
...
int tls_cli_session_connect(tls_cli_session *session)
{
    ...
    if ((ret = mbedtls_net_connect(&session->srv_fd, session->host, session->port,
                                   0)) != 0) {
        rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_net_connect err returned -0x%x\n", -ret);
        return ret;
    }

    rt_kprintf("[tls]");rt_kprintf("Connected %s:%s success...\n", session->host, session->port);
    mbedtls_ssl_set_bio(&session->ssl, &session->srv_fd, mbedtls_net_send, mbedtls_net_recv, 
# 260 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c" 3 4
                                                                                            (0)
# 260 "../../../packages/mbedtls-2.6.0/ports/src/tls_client.c"
                                                                                                );

    while ((ret = mbedtls_ssl_handshake(&session->ssl)) != 0) {
        if (ret != -0x6900 && ret != -0x6880) {
            rt_kprintf("[tls]");rt_kprintf("[fail]mbedtls_ssl_handshake returned -0x%x\n", -ret);
            return ret;
        }
    }

    if ((ret = mbedtls_ssl_get_verify_result(&session->ssl)) != 0) {
        rt_kprintf("[tls]");rt_kprintf("verify peer certificate fail....\n");
        memset(session->buffer, 0x00, session->buf_size);
        mbedtls_x509_crt_verify_info((char*)session->buffer, session->buf_size, "  ! ", ret);
        rt_kprintf("[tls]");rt_kprintf("verification info: %s\n", session->buffer);
    } else {
        rt_kprintf("[tls]");rt_kprintf("Certificate verified success...\n");
    }
