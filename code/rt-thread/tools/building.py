...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    # drivers build script
    objs.extend(SConscript(Rtt_Root + '/bsp/t6x0/drivers/SConscript', variant_dir=bsp_vdir + '/drivers', duplicate=0))
    # libraries build script
    objs.extend(SConscript(Rtt_Root +'/bsp/t6x0/libraries/SConscript', variant_dir=bsp_vdir + '/libraries', duplicate=0))
    ...
    if not has_libcpu:
        objs.extend(SConscript(Rtt_Root + '/libcpu/SConscript',
                    variant_dir=bsp_vdir + '/arch', duplicate=0))
    ...
    # include packages
    if os.path.exists(Rtt_Root + '/packages/SConscript'):
        objs.extend(SConscript(Rtt_Root + '/packages/SConscript',
                               variant_dir=bsp_vdir + '/packages',
                               duplicate=0))
    
    # include XXX TODO
    external_path = Rtt_Root + '/tools';
...
def DoBuilding(target, objects):
    ...
    if lib_name:
        ...
        if (Env['PLATFORM'] == 'win32') and (len(sys.argv) == 1) :
            #build using cdk make tool
            if rtconfig.CDK_PATH != '':
                os.system(rtconfig.CDK_ACTION)
        else:
...
def GenTargetProject(program = None):
    ...
    if GetOption('target') == 'cdk':
        ...
        CDKProject('project.cdkproj', Projects, Env['RTT_ROOT'])
...
def EndBuilding(target, program = None):
    ...
    if GetOption('make-dist') and program != None:
        ...
        if os.path.exists(os.path.join(BSP_ROOT,'dist')) :
            os.system('rm -fr '  + os.path.join(BSP_ROOT,'dist'))
        if os.path.exists(os.path.join(BSP_ROOT,'T6x0_RTOS_SDK_Vx.x.x')) :
            os.system('rm -fr '  + os.path.join(BSP_ROOT,'T6x0_RTOS_SDK_Vx.x.x'))
