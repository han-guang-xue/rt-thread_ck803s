...
static long _list_mutex(struct rt_list_node *list)
{
    ...
    for (node = list->next; node != list; node = node->next)
    {
       ...
       #if 0
       struct rt_list_node *thread_node;
       thread_node = &m->parent.suspend_thread;
       while (thread_node->next != &m->parent.suspend_thread)
       {
           struct rt_thread *thread;
           
           /* get thread entry */
           thread = rt_list_entry(thread_node->next, struct rt_thread, tlist);
