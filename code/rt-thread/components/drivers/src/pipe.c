...
static int __fast pipe_fops_open(struct dfs_fd *fd)
{
    rt_device_t device;
    rt_pipe_t *pipe;
...
static int __fast pipe_fops_close(struct dfs_fd *fd)
{
    rt_device_t device;
    rt_pipe_t *pipe;
...
static int __fast pipe_fops_ioctl(struct dfs_fd *fd, int cmd, void *args)
{
    rt_pipe_t *pipe;
    int ret = 0;
...
static int __fast pipe_fops_read(struct dfs_fd *fd, void *buf, size_t count)
{
    int len = 0;
    rt_pipe_t *pipe;
...
static int __fast pipe_fops_write(struct dfs_fd *fd, const void *buf, size_t count)
{
    int len;
    rt_pipe_t *pipe;
    int wakeup = 0;
...
static int __fast pipe_fops_poll(struct dfs_fd *fd, rt_pollreq_t *req)
{
    int mask = 0;
    rt_pipe_t *pipe;
    int mode = 0;
...
rt_err_t  __fast rt_pipe_open (rt_device_t device, rt_uint16_t oflag)
{
...
rt_err_t  __fast rt_pipe_close  (rt_device_t device)
{
    rt_pipe_t *pipe = (rt_pipe_t *)device;
...
rt_size_t __fast rt_pipe_read   (rt_device_t device, rt_off_t pos, void *buffer, rt_size_t count)
{
    uint8_t *pbuf;
...
rt_size_t __fast rt_pipe_write  (rt_device_t device, rt_off_t pos, const void *buffer, rt_size_t count)
{
    uint8_t *pbuf;
...
rt_err_t  __fast rt_pipe_control(rt_device_t dev, int cmd, void *args)
{
    return RT_EOK;
}
...
int __fast rt_pipe_delete(const char *name)
{
    int result = 0;
    rt_device_t device;
...
int __fast mkfifo(const char *path, mode_t mode)
{
    rt_pipe_t *pipe;
