...
#ifdef RT_USING_HEAP
rt_inline rt_err_t __fast _workqueue_work_completion(struct rt_workqueue *queue)
{
    rt_err_t result;
...
static void __fast _workqueue_thread_entry(void* parameter)
{
    rt_base_t level;
    struct rt_work* work;
...
struct rt_workqueue __fast *rt_workqueue_create(const char* name, rt_uint16_t stack_size, rt_uint8_t priority)
{
...
rt_err_t __fast rt_workqueue_destroy(struct rt_workqueue* queue)
{
    RT_ASSERT(queue != RT_NULL);
...
rt_err_t __fast rt_workqueue_dowork(struct rt_workqueue* queue, struct rt_work* work)
{
    rt_base_t level;
    RT_ASSERT(queue != RT_NULL);
...
rt_err_t __fast rt_workqueue_critical_work(struct rt_workqueue* queue, struct rt_work* work)
{
    rt_base_t level;
    RT_ASSERT(queue != RT_NULL);
    RT_ASSERT(work != RT_NULL);
...
rt_err_t __fast rt_workqueue_cancel_work(struct rt_workqueue* queue, struct rt_work* work)
{
    rt_base_t level;
...
rt_err_t __fast rt_workqueue_cancel_work_sync(struct rt_workqueue* queue, struct rt_work* work)
{
    rt_base_t level;
...
rt_err_t __fast rt_workqueue_cancel_all_work(struct rt_workqueue* queue)
{
