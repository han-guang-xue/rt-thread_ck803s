...
int __fast dfs_elm_read(struct dfs_fd *file, void *buf, size_t len)
{
    FIL *fd;
    FRESULT result;
    UINT byte_read;
...
int __fast dfs_elm_write(struct dfs_fd *file, const void *buf, size_t len)
{
    FIL *fd;
    FRESULT result;
    UINT byte_write;
...
#include <finsh.h>
MSH_CMD_EXPORT_ALIAS(mount_elm_ata, mt_elm_ata, mount ata(fat) to a path);
...
/* Read Sector(s) */
DRESULT __fast disk_read (BYTE drv, BYTE* buff, DWORD sector, UINT count)
{
    rt_size_t result;
...
/* Write Sector(s) */
DRESULT __fast disk_write (BYTE drv, const BYTE* buff, DWORD sector, UINT count)
{
    rt_size_t result;
    rt_device_t device = disk[drv];
...
DRESULT __fast disk_ioctl(BYTE drv, BYTE ctrl, void *buff)
{
    rt_device_t device = disk[drv];
...
int __fast ff_req_grant(_SYNC_t m)
{
...
void __fast ff_rel_grant(_SYNC_t m)
{
    rt_mutex_release(m);
}
