 ...
 * @note please don't invoke it on ISR.
 */
void __fast dfs_unlock(void)
{
    rt_mutex_release(&fslock);
}

static int __fast fd_alloc(struct dfs_fdtable *fdt, int startfd)
{
    int idx;
 ...
 * pointer.
 */
struct dfs_fd * __fast fd_get(int fd)
{
    struct dfs_fd *d;
    struct dfs_fdtable *fdt;
 ...
 */
void __fast fd_put(struct dfs_fd *fd)
{
    RT_ASSERT(fd != NULL);
 ...
 */
int __fast fd_is_open(const char *pathname)
{
    char *fullpath;
    unsigned int index;
 ...
 */
struct dfs_fdtable* __fast dfs_fdtable_get(void)
{
    struct dfs_fdtable *fdt;
#ifdef RT_USING_LWP
    struct rt_lwp *lwp;
