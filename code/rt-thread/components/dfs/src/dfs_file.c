 ...
 */
int __fast dfs_file_read(struct dfs_fd *fd, void *buf, size_t len)
{
    int result = 0;
 ...
 */
int __fast dfs_file_getdents(struct dfs_fd *fd, struct dirent *dirp, size_t nbytes)
{
    /* parameter check */
 ...
 */
int __fast dfs_file_write(struct dfs_fd *fd, const void *buf, size_t len)
{
    if (fd == NULL)
        return -EINVAL;
 ...
 * @return 0 on successful, -1 on failed.
 */
int __fast dfs_file_flush(struct dfs_fd *fd)
{
    if (fd == NULL)
        return -EINVAL;
 ...
 */
int __fast dfs_file_lseek(struct dfs_fd *fd, off_t offset)
{
    int result;
