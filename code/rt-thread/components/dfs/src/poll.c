...
static int __fast __wqueue_pollwake(struct rt_wqueue_node *wait, void *key)
{
...
static void __fast _poll_add(rt_wqueue_t *wq, rt_pollreq_t *req)
{
    struct rt_poll_table *pt;
...
static void __fast poll_table_init(struct rt_poll_table *pt)
{
...
static int __fast poll_wait_timeout(struct rt_poll_table *pt, int msec)
{
    rt_int32_t timeout;
    int ret = 0;
...
static int __fast do_pollfd(struct pollfd *pollfd, rt_pollreq_t *req)
{
    int mask = 0;
    int fd;
...
static int __fast poll_do(struct pollfd *fds, nfds_t nfds, struct rt_poll_table *pt, int msec)
{
    int num;
    int istimeout = 0;
    int n;
...
static void __fast poll_teardown(struct rt_poll_table *pt)
{
...
int __fast poll(struct pollfd *fds, nfds_t nfds, int timeout)
{
    int num;
