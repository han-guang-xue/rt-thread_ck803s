...
#if LWIP_MEM_DEBUG_RECORD
#define MEM_RECORD_MAX  100
void* mem_record[MEM_RECORD_MAX];
#endif
 ...
 */
err_t __fast sys_sem_new(sys_sem_t *sem, u8_t count)
{
 ...
 */
void __fast sys_sem_free(sys_sem_t *sem)
{
    RT_DEBUG_NOT_IN_INTERRUPT;
    rt_sem_delete(*sem);
}
 ...
 * Signal a semaphore
 */
void __fast sys_sem_signal(sys_sem_t *sem)
{
    rt_sem_release(*sem);
}
 ...
 */
u32_t __fast sys_arch_sem_wait(sys_sem_t *sem, u32_t timeout)
{
    rt_err_t ret;
    s32_t t;
    u32_t tick;
 ...
 */
int __fast sys_sem_valid(sys_sem_t *sem)
{
    return (int)(*sem);
}
#endif
 ...
 */
void __fast sys_sem_set_invalid(sys_sem_t *sem)
{
    *sem = RT_NULL;
}
#endif
 ...
 * @return a new mutex
 */
err_t __fast sys_mutex_new(sys_mutex_t *mutex)
{
    static unsigned short counter = 0;
    char tname[RT_NAME_MAX];
 ...
 */
void __fast sys_mutex_lock(sys_mutex_t *mutex)
{
    RT_DEBUG_NOT_IN_INTERRUPT;
 ...
 */
void __fast sys_mutex_unlock(sys_mutex_t *mutex)
{
 ...
 */
void __fast sys_mutex_free(sys_mutex_t *mutex)
{
    RT_DEBUG_NOT_IN_INTERRUPT;
 ...
 *  return 1 for valid, 0 for invalid
 */
int __fast sys_mutex_valid(sys_mutex_t *mutex)
{
    return (int)(*mutex);
}
#endif
 ...
 */
void __fast sys_mutex_set_invalid(sys_mutex_t *mutex)
{
    *mutex = RT_NULL;
}
#endif
 ...
 * @return the operation status, ERR_OK on OK; others on error
 */
err_t __fast sys_mbox_new(sys_mbox_t *mbox, int size)
{
    static unsigned short counter = 0;
    char tname[RT_NAME_MAX];
    sys_mbox_t tmpmbox;
 ...
 */
void __fast sys_mbox_free(sys_mbox_t *mbox)
{
    RT_DEBUG_NOT_IN_INTERRUPT;
 ...
 */
void __fast sys_mbox_post(sys_mbox_t *mbox, void *msg)
{
    RT_DEBUG_NOT_IN_INTERRUPT;
 ...
 */
err_t __fast sys_mbox_trypost(sys_mbox_t *mbox, void *msg)
{
 ...
 */
u32_t __fast sys_arch_mbox_fetch(sys_mbox_t *mbox, void **msg, u32_t timeout)
{
    rt_err_t ret;
    s32_t t;
    u32_t tick;
 ...
 */
u32_t __fast sys_arch_mbox_tryfetch(sys_mbox_t *mbox, void **msg)
{
    int ret;
 ...
 *  return 1 for valid, 0 for invalid
 */
int __fast sys_mbox_valid(sys_mbox_t *mbox)
{
    return (int)(*mbox);
}
#endif
 ...
 */
void __fast sys_mbox_set_invalid(sys_mbox_t *mbox)
{
    *mbox = RT_NULL;
}
#endif
...
sys_prot_t __fast sys_arch_protect(void)
{
    rt_base_t level;
...
void __fast sys_arch_unprotect(sys_prot_t pval)
{
    /* enable interrupt */
...
void __fast sys_arch_assert(const char *file, int line)
{
...
u32_t __fast sys_jiffies(void)
{
    return rt_tick_get();
}

u32_t __fast sys_now(void)
{
...
RT_WEAK
void RT_WEAK mem_init(void)
{
    #if LWIP_MEM_DEBUG_RECORD
    memset(mem_record, 0, sizeof(uint32) * MEM_RECORD_MAX);
    #endif
}

void * RT_WEAK mem_calloc(mem_size_t count, mem_size_t size)
{
    return rt_calloc(count, size);
}

void * RT_WEAK mem_trim(void *mem, mem_size_t size)
{
...
void * RT_WEAK __fast mem_malloc(mem_size_t size)
{
    ...
    if (mem_new != RT_NULL)
    {
        for (uint32 i = 0; i < MEM_RECORD_MAX; i++)
        {
            if (mem_record[i] == RT_NULL)
            {
                mem_record[i] = mem_new;
                break;
            }
        }
...
void  RT_WEAK __fast mem_free(void *mem)
{
    if (mem != RT_NULL)
    {
        #if LWIP_MEM_DEBUG_RECORD
